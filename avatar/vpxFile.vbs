...
'   2. Exponentiation: ^10 replaced with chain multiply in AudioFade, AudioPan
...
Const VolSli    = 3    ' Slingshot volume.
...
SolModCallback(26) = "AmpSuit"  ' Ampsuit mb flasher
...
Sub AmpSuit(m):m = m/255:ampprim.BlendDisableLighting = m +0.5:f26.state = m:end sub 
...
Sub sw7_Hit:vpmTimer.PulseSw 7:PlaySoundAtVol "Target", ActiveBall, VolTarg:End Sub
Sub sw8_Hit:vpmTimer.PulseSw 8:PlaySoundAtVol "Target", ActiveBall, VolTarg:End Sub
...
Sub sw42_Hit:vpmTimer.PulseSw 42:backbank.Y = 1:backbank.X = 1:Me.TimerEnabled = 1:PlaySoundAtVol "Target",ActiveBall, VolTarg:End Sub
Sub sw42_Timer:backbank.Y = 0.8888889:backbank.X = -0.8888889:Me.TimerEnabled = 0:End Sub
Sub sw43_Hit:vpmTimer.PulseSw 43:backbank.Y = 1:backbank.X = 1:Me.TimerEnabled = 1:PlaySoundAtVol "Target",ActiveBall, VolTarg:End Sub
Sub sw43_Timer:backbank.Y = 0.8888889:backbank.X = -0.8888889:Me.TimerEnabled = 0:End Sub
Sub sw44_Hit:vpmTimer.PulseSw 44:backbank.Y = 1:backbank.X = 1:Me.TimerEnabled = 1:PlaySoundAtVol "Target",ActiveBall, VolTarg:End Sub
    ...
    For b = 0 to ubBot
      If BallVel(BOT(b) ) > 1 Then
        ...
        if bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, Vol(BOT(b) )/7, AudioPan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) )
...
End If
if bzs < 24 Then BOT(b).velz = BOT(b).velz /20
    Next
End Sub
...
Sub Metals_Thin_Hit (idx)
	PlaySound "fx_metalhit2", 0, Vol(ActiveBall)*VolMetal, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
