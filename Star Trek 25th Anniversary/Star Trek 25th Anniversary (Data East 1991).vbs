...
Sub LampTimer_Timer()
    ...
    If Not IsEmpty(chgLamp) Then
        Dim cIdx, cVal  ' Cache 2D array dereferences
...
' Pre-built string arrays (eliminates per-frame string concatenation)
ReDim BallRollStr(10)
ReDim RampLoopStr(10)
Dim brsI : For brsI = 0 To 10 : BallRollStr(brsI) = "BallRoll_" & brsI : RampLoopStr(brsI) = "RampLoop" & brsI : Next
...
Sub RollingTimer_Timer()
        ...
        Dim bx, by, bz, bvx, bvy, bvz, bv, rvol, rpitch
        ...
        For b = 0 to ubBot
                ...
                If bv > 1 Then
					rolling(b) = True
					' Inline VolPlayfieldRoll and PitchPlayfieldRoll (avoid re-reading COM)
					rvol = RollingSoundFactor * 0.0005 * Csng(bv * bv * bv)
					rpitch = bv * bv * 15
						If bz < 30 Then
							PlaySound BallRollStr(b), -1, rvol * 1.1 * VolumeDial, AudioPan(BOT(b)), 0, rpitch, 1, 0, AudioFade(BOT(b))
							StopSound RampLoopStr(b)
						Else
							PlaySound RampLoopStr(b), -1, rvol * 2.5 * VolumeDial, AudioPan(BOT(b)), 0, rpitch, 1, 0, AudioFade(BOT(b))
							StopSound BallRollStr(b)
						End If
                Else
                ...
                '***Ball Drop Sounds*** (use cached bvz/bz)
                If bvz < -1 Then
                    If bz < 55 Then
                        If bz > 27 Then
...
Dim lastFTLF : lastFTLF = -9999
Dim lastFTRF : lastFTRF = -9999

sub FlipperTimer_Timer()
	' Cache currentangle and guard writes
	Dim aLF : aLF = LeftFlipper.currentangle
	If aLF <> lastFTLF Then
		lastFTLF = aLF
		FlipperLSh.RotZ = aLF
		LFLogo001.Roty = aLF
	End If
	Dim aRF : aRF = RightFlipper.currentangle
	If aRF <> lastFTRF Then
		lastFTRF = aRF
		FlipperRSh.RotZ = aRF
		RFLogo001.Roty = aRF
	End If
...
RightFlipper.timerinterval=10  ' Was 1ms (1000Hz). 10ms (100Hz) is visually identical, saves ~4500 calls/sec
...
Sub FlipperNudge(BOT, Flipper1, Endangle1, EOSNudge1, Flipper2, EndAngle2)
	Dim b
	' BOT passed in from caller (shared GetBalls)
	Dim ca1 : ca1 = Flipper1.currentangle  ' Cache COM read
...
Function FlipperTrigger(ballx, bally, Flipper)
	' Cache all flipper COM properties once
	Dim fx, fy, fca, flen
	...
	Dim frad : frad = (fca + 90) * PIover180
	Dim dfl : dfl = DistancePL(ballx, bally, fx, fy, Cos(frad)+fx, Sin(frad)+fy)
...
Sub FlipperTricks (Flipper, FlipperPress, FCount, FEndAngle, FState)
	' Cache COM properties once (were read 4+ times each)
	Dim sa : sa = Flipper.startangle
	Dim ca : ca = Flipper.currentangle
	Dim absSa : absSa = Abs(sa)
	Dim absCa : absCa = Abs(ca)

	Dim Dir
	Dir = sa / absSa        '-1 for Right Flipper

	If absCa > absSa - 0.05 Then  'If the flipper has started its swing, make it swing fast to nearly the end...
		If FState <> 1 Then
			...
			FState = 1
		End If
	ElseIf absCa <= Abs(Flipper.endangle) and FlipperPress = 1 then   'If the flipper is fully swung and the flipper button is pressed then
		...
		If FState <> 2 Then
			...
			FState = 2
		End If
	Elseif absCa > Abs(Flipper.endangle) + 0.01 and FlipperPress = 1 Then 'If the flipper has swung past it's end of swing then..
		If FState <> 3 Then													
...
Function AudioFade(tableobj) ' Fades between front and back of the table
  Dim tmp
    ...
    Else
        Dim nt1 : nt1 = -tmp
        Dim t2b,t4b,t8b : t2b=nt1*nt1 : t4b=t2b*t2b : t8b=t4b*t4b
...
' Pre-computed DynamicBSFactor powers (eliminates ^2 and ^3 per shadow per frame)
Dim DynBSFactor2 : DynBSFactor2 = DynamicBSFactor * DynamicBSFactor
Dim DynBSFactor3 : DynBSFactor3 = DynBSFactor2 * DynamicBSFactor
...
Sub DynamicBSUpdate
	...
	If ubBot < lob Then Exit Sub		'No balls in play, exit

	Dim invFalloff : invFalloff = 1 / falloff
	Dim falloffSq : falloffSq = falloff * falloff

'The Magic happens now
	Dim bx, by, bz  ' cached ball COM properties
	For s = lob to ubBot
		...
		If DynamicBallShadowsOn Then
			If bz < 30 Then
				...
				For iii = 0 to numberofsources - 1
					...
					dx = bx - sx : dy = by - sy
					dsq = dx*dx + dy*dy
					' Distance-squared gate: skip Sqr/DistanceFast for distant sources
					If dsq < falloffSq And gilvl > 0 Then
						...
						Elseif currentShadowCount(s) = 2 Then
							...
							Dim asx, asy : asx = DSSources(AnotherSource)(0) : asy = DSSources(AnotherSource)(1)
