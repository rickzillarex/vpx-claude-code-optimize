...
Dim RFUp
Dim lastLFAngle, lastLFUpAngle, lastRFAngle, lastRFUpAngle
lastLFAngle = -999 : lastLFUpAngle = -999 : lastRFAngle = -999 : lastRFUpAngle = -999
...
Sub FlipperTimer_Timer
	...
	Dim caLF, caLFUp, caRF, caRFUp
	caLF = FlipperLeft.CurrentAngle
	caLFUp = FlipperLeftUp.CurrentAngle
	caRF = FlipperRight.CurrentAngle
	caRFUp = FlipperRightUp.CurrentAngle

	If caLF <> lastLFAngle Then
		FlipperLeftGottlieb.objRotz = caLF - 90
		FlipperLeftSh.RotZ = caLF
		lastLFAngle = caLF
	End If
	If caLFUp <> lastLFUpAngle Then
		FlipperLeftUpGottlieb.objRotz = caLFUp - 90
		FlipperLeftUpSh.RotZ = caLFUp
		lastLFUpAngle = caLFUp
	End If
	If caRF <> lastRFAngle Then
		FlipperRightGottlieb.objRotz = caRF - 90
		FlipperRightSh.RotZ = caRF
		lastRFAngle = caRF
	End If
	If caRFUp <> lastRFUpAngle Then
		FlipperRightUpGottlieb.objRotz = caRFUp - 90
		lastRFUpAngle = caRFUp
	End If
End Sub
...
Sub LampTimer_Timer()
    ...
    If Not IsEmpty(chgLamp) Then
        ...
        For ii = 0 To UBound(chgLamp)
           ...
           'Special Handling
           If cIdx = 0 Then SolRelayQ cVal 'InGame Q Relay
           If cIdx = 1 Then SolRelayT cVal 'Tilt T Relay
           If cIdx = 2 Then SolBallRelease cVal
           'If cIdx = 4 Then SolSound16 cVal 'Sound 16?
           If cIdx = 12 Then SolBallShooter cVal
           If cIdx = 13 Then SolRelayS cVal 'Switching S Relay
           'If cIdx = 14 Then SolRelayA cVal 'Playboard Aux Lamp A Relay
           If cIdx = 15 Then SolRelayB cVal 'Lightbox Aux Lamp B Relay  aka sequentitial light show
           If cIdx = 16 Then SolLeftBallGate cVal
           If cIdx = 17 Then SolRightBallGate cVal
           'If cIdx = 18 Then SolRelayC cVal 'Skull Motor C Relay
           'If cIdx = 19 Then Topper cVal ' Skull Jaw Relay
        Next
    End If
    UpdateLamps
End Sub
...
Sub RollingTimer_Timer()
    ...
    Dim ball, bx, by, bz, bvx, bvy, bvz, bv, ballvol, ballpitch
    ...
    For b = 0 to ub
        ...
        ' inlined VolPlayfieldRoll
        ballvol = bv / 6
        If ballvol > 1 Then ballvol = 1

        ' inlined PitchPlayfieldRoll
        ballpitch = ballvol * ballvol * 15000

        ' inlined AudioPan
        Dim panTmp, panVal
        panTmp = bx * InvTWHalf - 1
        If panTmp < 0 Then
            Dim panN : panN = -panTmp
            Dim panN2 : panN2 = panN * panN : Dim panN4 : panN4 = panN2 * panN2
            Dim panN8 : panN8 = panN4 * panN4
            panVal = -(panN8 * panN2)
        Else
            Dim panP : panP = panTmp
            Dim panP2 : panP2 = panP * panP : Dim panP4 : panP4 = panP2 * panP2
            Dim panP8 : panP8 = panP4 * panP4
            panVal = panP8 * panP2
        End If

        ' inlined AudioFade
        Dim fadeTmp, fadeVal
        fadeTmp = by * InvTHHalf - 1
        If fadeTmp < 0 Then
            Dim fadeN : fadeN = -fadeTmp
            Dim fadeN2 : fadeN2 = fadeN * fadeN : Dim fadeN4 : fadeN4 = fadeN2 * fadeN2
            Dim fadeN8 : fadeN8 = fadeN4 * fadeN4
            fadeVal = -(fadeN8 * fadeN2)
        Else
            Dim fadeP : fadeP = fadeTmp
            Dim fadeP2 : fadeP2 = fadeP * fadeP : Dim fadeP4 : fadeP4 = fadeP2 * fadeP2
            Dim fadeP8 : fadeP8 = fadeP4 * fadeP4
            fadeVal = fadeP8 * fadeP2
        End If

        If bv > 1 And bz < 30 Then
            rolling(b) = True
            PlaySound BallRollStr(b), -1, ballvol * 1.1 * VolumeDial, panVal, 0, ballpitch, 1, 0, fadeVal
        Else   'increase the pitch on a ramp
            rolling(b) = True
            PlaySound BallRollStr(b), -1, ballvol * 5.5 * VolumeDial, panVal, 0, ballpitch + 25000, 1, 0, fadeVal
        End If
...
FlipperRight.timerinterval=10   ' was 1ms, 10ms is visually identical and saves ~4500 calls/sec
...
sub FlipperRight_timer()
	...
	If LFPress = 0 And RFPress = 0 Then Exit Sub  ' skip nudge when flippers at rest
	Dim fBOT : fBOT = GetBalls
	FlipperNudge fBOT, FlipperRight, RFEndAngle, RFEOSNudge, FlipperLeft, LFEndAngle
	FlipperNudge fBOT, FlipperLeft, LFEndAngle, LFEOSNudge,  FlipperRight, RFEndAngle
end sub
...
' Pre-computed inverse multipliers (eliminates per-call division)
Dim InvTWHalf : InvTWHalf = 2 / tablewidth
Dim InvTHHalf : InvTHHalf = 2 / tableheight
Dim TW_d2 : TW_d2 = tablewidth / 2
Dim BS_d6 : BS_d6 = Ballsize / 6
Dim InvTW7 : InvTW7 = 1 / 7

' Pre-built string arrays for rolling sounds
ReDim BallRollStr(5)
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	Dim velF : velF = Csng(velocity) : PlaySound (snd), 0, velF * velF / 200 * BallWithBallCollisionSoundFactor * VolumeDial, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
