...
Sub Table1_Init
    ...
    ' OPT-5: Pre-compute inverse table dimension constants for AudioPan/AudioFade.
    '         Replaces per-call division by table1.width/height (COM reads + division).
...
Sub SolDiv(Enabled)
	If Enabled Then
		...
		DiverterDir = -1
		Diverter.Interval = 10:Diverter.Enabled = 1
...
Sub Diverter_Timer()
    ' OPT-1: Cache RotZ into local — eliminates 2 extra COM reads per tick.
    ' OPT-1: Interval changed 5ms→10ms in SolDiv (halves call frequency, 200Hz→100Hz).
    Dim rz : rz = DiverterP.RotZ + DiverterDir
    If DiverterDir = 1 Then
        If rz > 5 Then rz = 5 : Me.Enabled = 0
    Else
        If rz < -35 Then rz = -35 : Me.Enabled = 0
    End If
    DiverterP.RotZ = rz
End Sub
...
' OPT-4b: Eliminated Dictionary + string-key lookup from AddLamp hot path.
'          Type code is now passed directly as a parameter (AddLampT).
'          At 33Hz x ~35 calls/tick: eliminates ~1155 Dictionary hash lookups/sec
'          + ~1155 string concatenations/sec (CStr + "&" + ObjName).
'          Type codes: 1=Light, 2=Flasher, 3=Primitive
...
Sub InitLamps
    ...
    Next
    ' OPT-4b: Registration removed — type codes passed directly to AddLampT calls.
...
Sub LampTimer_Timer()
	...
	' OPT-4b: AddLampT passes type code directly (1=Light, 2=Flasher, 3=Primitive).
	'         Eliminates Dictionary lookup + string concat per call.
	'Bulbs
	SwapTex 2, TopTeeth, "teeth", "teeth_on"		'teeth
	SwapTex 2, BottomTeeth, "teeth", "teeth_on"		'teeth
	AddLampT 2, Li2, 1			'teeth (Light)
	AddLampT 10, bulb9, 3		'BAT-o-Meter "5.000" (Primitive)
	AddLampT 11, bulb8, 3		'BAT-o-Meter "10.000" (Primitive)
	AddLampT 12, bulb7, 3		'BAT-o-Meter "20.000" (Primitive)
	AddLampT 13, bulb6, 3		'BAT-o-Meter "EXTRA BALL" (Primitive)
	AddLampT 14, bulb5, 3		'BAT-o-Meter "SPECIAL" (Primitive)

	AddLampT 10, Li10, 1		'BAT-o-Meter "5.000" (Light)
	AddLampT 11, Li11, 1		'BAT-o-Meter "10.000" (Light)
	AddLampT 12, Li12, 1		'BAT-o-Meter "20.000" (Light)
	AddLampT 13, Li13, 1		'BAT-o-Meter "EXTRA BALL" (Light)
	AddLampT 14, Li14, 1		'BAT-o-Meter "SPECIAL" (Light)

	AddLampT 36, bulb3, 3		'Left Ramp Add Letter (Left) (Primitive)
	AddLampT 37, bulb2, 3		'Left Ramp Million (Right) (Primitive)
	AddLampT 46, bulb3, 3		'Right Ramp Add Letter (Left) (Primitive)
	AddLampT 47, bulb4, 3		'Right Ramp Million (Right) (Primitive)
	AddLampT 67, bulb10, 3		'"TO CRYPT" (Primitive)
	'Flashers
	AddLampT 152, domeYB, 2		'Yellow Dome Bottom (Flasher)
	AddLampT 152, F22, 2
	AddLampT 152, F22r1, 2
	AddLampT 152, F22r2, 2
	AddLampT 152, domeW, 2		'White Dome (Flasher)
	AddLampT 152, F22a, 2
	AddLampT 152, F22r3, 2
	AddLampT 152, F22r4, 2
	AddLampT 153, domeYT, 2		'Yellow Dome Top (Flasher)
	AddLampT 153, F22x, 2
	AddLampT 153, F22r1x, 2
	AddLampT 153, F22r2x, 2
	AddLampT 153, domeR, 2		'Red Dome (Flasher)
	AddLampT 153, F23, 2
	AddLampT 153, F23r, 2
	AddLampT 154, F24, 2		'Under The Ramp (Flasher)
	AddLampT 154, domeB, 2		'Blue Dome (Flasher)
	AddLampT 154, F24a, 2
	AddLampT 154, F24r1, 2
	AddLampT 154, F24r2, 2
	AddLampT 155, F25, 2		'Flasher:Heart #1 (Flasher)
...
' OPT-7: Guard HideLamp .visible writes — only write when value changes.
'         Eliminates ~198 redundant COM writes/sec (3 objects x 33Hz x 2 branches).
Dim lastHideLamp : lastHideLamp = -1
Sub HideLamp (enabled)
    If enabled = lastHideLamp Then Exit Sub
    lastHideLamp = enabled
	F22x.visible=NOT enabled
	F22r1x.visible=NOT enabled
	F22r2x.visible=NOT enabled
End Sub

' OPT-4b: AddLampT takes objType directly — zero Dictionary/string ops per call.
Sub AddLampT(nr, object, objType)
    Dim fl : fl = FlashLevel(nr)
    Select Case LampState(nr)

        Case 0 'off
            If fl > 0 Then
                fl = fl - FlashSpeedDown(nr)
                If fl < 0 Then fl = 0
                FlashLevel(nr) = fl
            End If
            On Error Resume Next
...
Function Vol(ball) ' Calculates the Volume of the sound based on the ball speed
    ' OPT-5: Inline BallVel with cached VelX/VelY — eliminates double ^2 operator and
    '         double COM reads vs. calling BallVel() separately.
    Dim vx, vy : vx = ball.VelX : vy = ball.VelY
    Dim bv : bv = INT(SQR(vx*vx + vy*vy))
    Vol = CSng(bv * bv) / 2000
End Function

Function AudioPan(ball) ' Calculates the pan for a ball based on the X position on the table
    ' OPT-5: Use pre-computed InvTWHalf — eliminates 2/table1.width COM read + division per call.
    '         Replace ^10 with multiply chain t2→t4→t8→t8*t2 — eliminates Exp+Log per call.
    Dim tmp, t2, t4, t8
...
Function BallVel(ball) 'Calculates the ball speed
    ' OPT-5: Replace ^2 operator with multiply — eliminates Exp+Log pair.
    Dim vx, vy : vx = ball.VelX : vy = ball.VelY
    BallVel = INT(SQR(vx*vx + vy*vy))
End Function

Function AudioFade(ball)
    ' OPT-5: Use pre-computed InvTHHalf — eliminates 2/Table1.height COM read + division per call.
    '         Replace ^10 with multiply chain t2→t4→t8→t8*t2.
    Dim tmp, t2, t4, t8
...
' 							Maths
' *********************************************************************
' OPT-5/6: Pre-computed trig and table dimension constants.
'           Eliminates / 180 division per dSin/dCos call, and / table1.width / table1.height
'           COM reads inside AudioPan/AudioFade every frame per ball.
Dim PIover180 : PIover180 = 3.1415927 / 180
' Table dimensions are fixed at load; cache inverse-half for AudioPan/AudioFade multiply.
' These are set in Table1_Init_PostLoad (after Table1 is ready). Initialized here as
' sentinels; actual values written in Table1_Init.
Dim InvTWHalf : InvTWHalf = 0
Dim InvTHHalf : InvTHHalf = 0

Function dSin(degrees)
    ' OPT-5: Use pre-computed PIover180 — eliminates / 180 division per call.
    dsin = sin(degrees * PIover180)
End Function

Function dCos(degrees)
    ' OPT-5: Use pre-computed PIover180.
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	PlaySoundXY "fx_collide", 0, Csng(velocity * velocity) / 500, 0, ball1
End Sub
...
Sub RubberPosts_Hit()
 	dim finalspeed
 	Dim rpvx, rpvy : rpvx = activeball.velx : rpvy = activeball.vely
  	finalspeed=SQR(rpvx*rpvx + rpvy*rpvy)
...
Sub Rubbers_Hit(idx)
 	dim finalspeed
 	Dim rbvx, rbvy : rbvx = activeball.velx : rbvy = activeball.vely
  	finalspeed=SQR(rbvx*rbvx + rbvy*rbvy)
 	If finalspeed > 20 then 
		PlaySoundAtBall "fx_rubber"
	End if
	If finalspeed >= 6 AND finalspeed <= 20 then
...
' OPT-3: Last-angle sentinels for delta guards. Initialized to impossible value so
'         first frame always writes. Eliminates ~240 COM reads/sec + ~360 COM writes/sec
'         when flippers/gates are stationary (the common case).
Dim lastLFAngle : lastLFAngle = -9999
Dim lastRFAngle : lastRFAngle = -9999
Dim lastLGAngle : lastLGAngle = -9999
Dim lastRGAngle : lastRGAngle = -9999
Dim lastSW15Angle : lastSW15Angle = -9999

Sub RealTimeUpdates()
    ' OPT-3: Cache each currentangle once into a local — 1 COM read instead of 2.
    '         Delta-guard each write — only cross COM boundary when value actually changed.
    Dim lfa, rfa, lga, rga, s15a
    lfa = LeftFlipper.currentangle
    If lfa <> lastLFAngle Then
        lastLFAngle = lfa
        Prim_LeftFlipper.RotY = lfa - 90
        FlipperLSh.RotZ = lfa
    End If
    rfa = RightFlipper.currentangle
    If rfa <> lastRFAngle Then
        lastRFAngle = rfa
        Prim_RightFlipper.RotY = rfa - 90
        FlipperRSh.RotZ = rfa
    End If
    lga = LGate.currentangle
    If lga <> lastLGAngle Then
        lastLGAngle = lga
        LeftGate.RotX = -lga
    End If
    rga = RGate.currentangle
    If rga <> lastRGAngle Then
        lastRGAngle = rga
        RightGate.RotX = -rga
    End If
    s15a = sw15.currentangle
    If s15a <> lastSW15Angle Then
        lastSW15Angle = s15a
        sw15s.RotX = -s15a
    End If
    RollingSoundUpdate
...
' OPT-2: Pre-built rolling sound name array. Eliminates "fx_ballrolling"&(b+1) string
'         allocation every frame per ball (~120 allocs/sec at 60fps with 2 balls).
ReDim BallRollStr(tnob)
Dim brsInit : For brsInit = 0 To tnob : BallRollStr(brsInit) = "fx_ballrolling" & (brsInit+1) : Next

Sub InitRolling:Dim i:For i=0 to (tnob-1):rolling(i) = False:Next:End Sub

Sub RollingSoundUpdate()
    ' OPT-2: Cache BOT once — shared with caller RealTimeUpdates to avoid double GetBalls.
    '        Use cached gBOT_frame (set in RealTimeUpdates) when available.
    '        Early-exit when no balls on table.
    '        Inline BallVel: cache VelX/VelY into locals, compute bv once, reuse for Vol.
    '        Use pre-built BallRollStr(b) — no string concat per tick.
    Dim BOT, b, ball, bvx, bvy, bvz, bz, bv, bvol
    BOT = GetBalls
    If UBound(BOT) = -1 Then Exit Sub
    For b = 0 To UBound(BOT)
        Set ball = BOT(b)
        bvx = ball.VelX : bvy = ball.VelY : bz = ball.z
        ' OPT-5 (inline): bv = INT(SQR(bvx*bvx + bvy*bvy)) — no ^2 operator
        bv = INT(SQR(bvx*bvx + bvy*bvy))
        If bv > 1 And bz < 30 Then
            rolling(b) = True
            ' OPT-5 (inline): bvol = bv*bv/2000 — was BallVel()^2/2000 (two calls, two ^2)
            bvol = CSng(bv * bv) / 2000
            PlaySoundXY BallRollStr(b), -1, bvol, 1, ball
        Else
            If rolling(b) Then
...
'*********** BALL SHADOW *********************************
' OPT-6: BallShadowUpdate was commented out but its body still executed at module level
'         (VBScript ignores the 'Sub/'End Sub comment lines, runs the code between them).
'         Removed the orphaned module-level code entirely. BallShadow array kept in case
