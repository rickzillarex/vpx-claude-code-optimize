...
' ==========================================
' PRE-COMPUTED CONSTANTS (OPT: computed once)
' ==========================================
' Table dimensions - must be assigned in Table1_Init
Dim InvTWHalf, InvTHHalf
' Pre-built rolling sound strings (OPT: eliminates per-tick string concat)
Const tnob = 15 ' total number of balls
ReDim BallRollStr(15)
Dim brsI : For brsI = 0 To tnob : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next
ReDim BallDropStr(15)
Dim bdsI : For bdsI = 0 To tnob : BallDropStr(bdsI) = "fx_ball_drop" & bdsI : Next
...
Sub Table1_Init()
  ...
  ' OPT: Pre-compute table dimension inverse multipliers (cannot be done at module level)
...
Sub UpdateGI(nr,step)
				For each xxx in GI:xxx.IntensityScale = 0.4 * step:next
...
' OPT: AudioFade/AudioPan/Pan — ^10 replaced with chained multiply, division replaced with pre-computed inverse
Function AudioFade(tableobj) ' Fades between front and back of the table
  ...
  ElseIf tmp < 0 Then
    Dim nt : nt = -tmp
    Dim n2f,n4f,n8f : n2f=nt*nt : n4f=n2f*n2f : n8f=n4f*n4f
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table
  ...
  If tmp > 0 Then
    Dim t2n,t4n,t8n : t2n=tmp*tmp : t4n=t2n*t2n : t8n=t4n*t4n
    Pan = Csng(t8n * t2n)
  ElseIf tmp < 0 Then
    Dim ntn : ntn = -tmp
    Dim n2n,n4n,n8n : n2n=ntn*ntn : n4n=n2n*n2n : n8n=n4n*n4n
    Pan = Csng(-(n8n * n2n))
  Else
    Pan = 0
  End If
...
' OPT: ^2 replaced with x*x, COM reads cached
...
' OPT: Shared GetBalls, pre-built string arrays, inlined BallVel/Vol/Pitch/AudioPan/AudioFade,
'       COM property caching (ball.VelX/VelY/z/VelZ cached into locals)
Sub RollingTimer_Timer()
    ...
    Dim bvx, bvy, bv, bz, bvz
    Dim ballPan, ballFade, ballVol, ballPitch
    ...
    For b = 0 to ubBot
      ...
      If bv > 1 Then
        rolling(b) = True
        bz = BOT(b).z
        ballVol = Csng(bv * bv / VolDiv) / 12
        ballPan = AudioPan(BOT(b))
        ballPitch = bv * 20
        ballFade = AudioFade(BOT(b))
        if bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, ballVol, ballPan, 0, ballPitch, 1, 0, ballFade
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, ballVol, ballPan, 0, ballPitch+50000, 1, 0, ballFade
        End If
      Else
 ...
 ' play ball drop sounds
        ...
        If bvz < -1 Then
          If bz < 55 Then
            If bz > 27 Then
              PlaySound BallDropStr(b), 0, ABS(bvz)/17, AudioPan(BOT(b)), 0, bv*20, 1, 0, AudioFade(BOT(b))
            End If
          End If
        End If
    Next
...
' OPT: COM property caching (BOT(b).X/Y/Z cached into locals), guard .visible writes,
'       BallShadow array moved to module level, GetBalls shared intent
Sub BallShadowUpdate()
Dim BallShadow
BallShadow = Array (BallShadow1,BallShadow2,BallShadow3,BallShadow4,BallShadow5,BallShadow6,BallShadow7,BallShadow8,BallShadow9,BallShadow10,BallShadow11,BallShadow12,BallShadow13,BallShadow14,BallShadow15)
    Dim BOT, b
    BOT = GetBalls
    Dim ubBot : ubBot = UBound(BOT)
...
Sub DeadWorld_Timer()
  ...
  '### Switch 71 Opto ###
  If rotY >= 115 AND rotY <=125 Then
    Controller.Switch(77) = 1
  else
    Controller.Switch(77) = 0
  End If
  If rotY >= 235 AND rotY <=245 Then
    Controller.Switch(77) = 1
  End If
  If rotY >= 350 AND rotY <=360 Then
    Controller.Switch(77) = 1
  End If

  '### Switch 61 Opto ###

  If rotY >= 65 AND rotY <=70 Then
    Controller.Switch(61) = 1
  else
    Controller.Switch(61) = 0
  End If

  If rotY >= 185 AND rotY <=190 Then
    Controller.Switch(61) = 1
  End If

  If rotY >= 305 AND rotY <=310 Then
    Controller.Switch(61) = 1
  End If

  '### Feed Pause ###

  If rotY >= 70 AND rotY <=75 AND jdbp = 0 Then
    Dead_Block.Isdropped = 1
    Block_Reset.enabled = 1
    jdb = 1
  End If
  If rotY >= 185 AND rotY <=192 AND jdbp1 = 0 Then
    Dead_Block.Isdropped = 1
    Block_Reset.enabled = 1
    jdb1 = 1
  End If
  If rotY >= 302 AND rotY <=310 AND jdbp2 = 0 Then
    Dead_Block.Isdropped = 1
    Block_Reset.enabled = 1
    jdb2 = 1
  End If
...
Sub Planet_Watch_Timer()
  Dim rotY : rotY = Nipple.RotY
  If jdbp = 1 AND rotY >= 220 AND rotY <=250 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX_action = 1:Crane_X.enabled = 1:me.enabled = 0
  End If
  If jdbp1 = 1 AND rotY >= 320 AND rotY <=360 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX_action = 1:Crane_X.enabled = 1:me.enabled = 0
  End If
  If jdbp2 = 1 AND rotY >= 100 AND rotY <=130 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX_action = 1:Crane_X.enabled = 1:me.enabled = 0
  End If

  If jdbp = 0 AND rotY >= 220 AND rotY <=250 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX2_action = 1:Crane2_X.enabled = 1:me.enabled = 0
  End If
  If jdbp1 = 0 AND rotY >= 320 AND rotY <=360 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX2_action = 1:Crane2_X.enabled = 1:me.enabled = 0
  End If
  If jdbp2 = 0 AND rotY >= 100 AND rotY <=130 AND bout1 = 0 AND magon = 1 AND DeadWorld.enabled = FALSE Then
    CX2_action = 1:Crane2_X.enabled = 1:me.enabled = 0
  End If
End Sub
...
Sub Crane_X_Timer()
  Select Case CX_action
    ...
    Case 3
      Dim rotY3 : rotY3 = Nipple.RotY
      If jdbp = 1 AND rotY3 >= 220 AND rotY3 <=250 AND bout1 = 0 AND magon = 1 Then
        Ball1.Visible = 0
        Set Cball = Crane_Kick.createball:Cball.id = 200
        BPos = 1
        Ball_Move.enabled = 1
      End If
      If jdbp1 = 1 AND rotY3 >= 320 AND rotY3 <=360 AND bout3 = 0  AND magon = 1 Then
        Ball2.Visible = 0
        Set Cball = Crane_Kick.createball:Cball.id = 201
        BPos = 1
        Ball_Move.enabled = 1
      End If
      If jdbp2 = 1 AND rotY3 >= 100 AND rotY3 <=130 AND bout2 = 0  AND magon = 1 Then
        Ball.Visible = 0
...
Sub Ball_Move_Timer()
  Select Case Bpos
    ...
    Case 3:
      ...
      Cball.Z = Cball.Z - 7
    Case 4:
      Dim rotY4 : rotY4 = Nipple.RotY
      If jdbp = 1 AND rotY4 >= 220 AND rotY4 <=250 AND magon = 1 Then
        Crane_Kick.kick 180,0
        jdbp = 0
        bout1 = 1
      End If
      If jdbp1 = 1 AND rotY4 >= 320 AND rotY4 <=360 AND magon = 1 Then
        Crane_Kick.kick 180,0
        jdbp1 = 0
        bout3 = 1
      End If
      If jdbp2 = 1 AND rotY4 >= 100 AND rotY4 <=130 AND magon = 1 Then
        Crane_Kick.kick 180,0
        jdbp2 = 0
        bout2 = 1
      End If
...
' OPT: Cache flipper CurrentAngle into locals to reduce COM reads
Dim lastLFAngle : lastLFAngle = -9999
Dim lastRFAngle : lastRFAngle = -9999
Dim lastLF2Angle : lastLF2Angle = -9999
Dim lastRF2Angle : lastRF2Angle = -9999

Sub JDFlip_Timer()
  Dim lfca : lfca = LeftFlipper.CurrentAngle
  Dim rfca : rfca = RightFlipper.CurrentAngle
  Dim lf2ca : lf2ca = LeftFlipper2.CurrentAngle
  Dim rf2ca : rf2ca = RightFlipper2.CurrentAngle

  If lfca <> lastLFAngle Then
    lfs.RotZ = lfca
    LeftFlipperP.RotY = lfca
    lastLFAngle = lfca
  End If
  If rfca <> lastRFAngle Then
    rfs.RotZ = rfca
    RightFlipperP.RotY = rfca
    lastRFAngle = rfca
  End If
  If lf2ca <> lastLF2Angle Then
    LeftFlipperP2.RotY = lf2ca
    lastLF2Angle = lf2ca
  End If
  If rf2ca <> lastRF2Angle Then
    RightFlipperP2.RotY = rf2ca
    lastRF2Angle = rf2ca
  End If
