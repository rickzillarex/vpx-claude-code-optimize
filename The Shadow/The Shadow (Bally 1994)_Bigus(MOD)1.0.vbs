...
Const VolDiv = 2000    ' Lower number, louder ballrolling/collision sound
...
if Table1.showDT = False then ramp14.visible = 0:ramp15.visible = 0
...
' Pre-computed inverse half-dimensions for AudioFade/AudioPan/Pan (assigned in Table1_Init)
Dim InvTWHalf, InvTHHalf

' Pre-built sound string arrays (assigned in Table1_Init)
Dim BallRollStr(), MetalRollStr(), BallDropStr()

' Flipper shadow guarded-write state
Dim lastLFAngle : lastLFAngle = -9999
Dim lastRFAngle : lastRFAngle = -9999
Dim lastRF2Angle : lastRF2Angle = -9999

' Flipper logo guarded-write state
Dim lastLFLogoAngle : lastLFLogoAngle = -9999
Dim lastRFLogoAngle : lastRFLogoAngle = -9999
Dim lastRF2LogoAngle : lastRF2LogoAngle = -9999

' HandleMiniPF flipper/gate angle cache
Dim lastMiniLF : lastMiniLF = -9999
Dim lastMiniRF : lastMiniRF = -9999
Dim lastMiniRF2 : lastMiniRF2 = -9999
Dim lastMiniG1 : lastMiniG1 = -9999
Dim lastMiniG2 : lastMiniG2 = -9999

' BallShadow array (built once at module level, not per frame)
Dim BallShadow

 Sub Table1_Init
    ' Compute inverse half-dimensions for audio functions
    InvTWHalf = 2 / table1.width
    InvTHHalf = 2 / table1.height

    ' Pre-build sound string arrays
    Dim si
    ReDim BallRollStr(tnob)
    ReDim MetalRollStr(tnob)
    ReDim BallDropStr(tnob)
    For si = 0 To tnob
        BallRollStr(si) = "fx_ballrolling" & si
        MetalRollStr(si) = "fx_metalrolling" & si
        BallDropStr(si) = "fx_ball_drop" & si
    Next

    ' Build BallShadow array once
...
Sub UpdateFlipperLogos_Timer
  ...
  If aLF <> lastLFLogoAngle Then lastLFLogoAngle = aLF : batleft.ObjRotZ = aLF
  Dim aRF : aRF = RightFlipper.CurrentAngle
  If aRF <> lastRFLogoAngle Then lastRFLogoAngle = aRF : batright.ObjRotZ = aRF
  Dim aRF2 : aRF2 = RightFlipper2.CurrentAngle
  If aRF2 <> lastRF2LogoAngle Then lastRF2LogoAngle = aRF2 : batright1.ObjRotZ = aRF2
End Sub
...
'PHURBA DIVERTERS *****************************************************************************************************************************************
 ...
 Divertershadow_left.TransX= -42
 Divertershadow_left.TransZ= 0
 Divertershadow_left.TransY= 21
 Divertershadow_left.RotY= 9
 Divertershadow_left.ObjRotZ= 48
 ...
 Divertershadow_left.TransZ= 0
 Divertershadow_left.TransX= 0
 Divertershadow_left.TransY= 0
 Divertershadow_left.ObjRotZ= 0
 ...
 Divertershadow_Right.TransX= 0
 Divertershadow_Right.TransZ= 0
 Divertershadow_Right.TransY= -3
 Divertershadow_Right.RotY= -5
 Divertershadow_Right.RotZ= 31
 Divertershadow_Right.ObjRotZ= 0
...
' RightPhurbaLeft.isdropped = False
 ...
 Divertershadow_Right.TransX= 57
 Divertershadow_Right.TransY= 10
 Divertershadow_Right.TransZ= 0
 Divertershadow_Right.RotY= 0
 Divertershadow_Right.RotZ= -7
 Divertershadow_Right.ObjRotZ= 0
...
Divertershadow_Right.transx=61
...
Divertershadow_Right.transx=54.5
...
Divertershadow_Right.transx=2.5
...
Divertershadow_left.transx=-2.5
...
NFadeLm 117, F117a
 ...
 Flashm 118, F118C
 Flashm 118, F118C_ref
 Flash 118, F118
 Flashm 121, F121A
 Flashm 121, F121B
 Flashm 121, F121C
 Flashm 121, F121D
 Flashm 121, F121D_ref
...
NFadeLm 128, F128b
 Flashm 128, F128_ref
 Flash 128, F128
NFadeObjm 81, AnelloSxRampaDx, "shadowringlit", "shadowring"
 Flashm 81, F181_ref
 Flashm 81, F181_ref2
 Flash 81, F181
NFadeObjm 82, AnelloDxRampaDx, "shadowringlit", "shadowring"
 Flashm 82, F182_ref
 Flash 82, F182
NFadeObjm 83, AnelloDxRampaDx1, "shadowringlit", "shadowring"
 Flashm 83, F183_ref
 Flash 83, F183
NFadeObjm 84, AnelloDxRampaSx, "shadowringlit", "shadowring"
 Flashm 84, F184_ref
 Flash 84, F184
  End Sub
...
Sub RollingTimer_Timer()
  ...
  ' play the rolling sound for each ball
    Dim ball, bvx, bvy, bvz, bz, bx, by, bv, bvSq, bPitch, bPan, bFade
    For b = 0 to ubBot
        Set ball = BOT(b)
        bvx = ball.VelX : bvy = ball.VelY : bvz = ball.VelZ
        bz = ball.z : bx = ball.x : by = ball.y
        bv = INT(SQR(bvx * bvx + bvy * bvy))
        bvSq = bv * bv
        bPitch = bv * 20

        ' Inline Pan
        Dim ptmp : ptmp = bx * InvTWHalf - 1
        If ptmp > 0 Then
            Dim p2,p4,p8 : p2=ptmp*ptmp : p4=p2*p2 : p8=p4*p4
            bPan = Csng(p8 * p2)
        ElseIf ptmp < 0 Then
            Dim pn,pn2,pn4,pn8 : pn=-ptmp : pn2=pn*pn : pn4=pn2*pn2 : pn8=pn4*pn4
            bPan = Csng(-(pn8 * pn2))
        Else
            bPan = 0
        End If
        ...
        ' Inline AudioFade
        Dim ftmp : ftmp = by * InvTHHalf - 1
        If ftmp > 0 Then
            Dim f2,f4,f8 : f2=ftmp*ftmp : f4=f2*f2 : f8=f4*f4
            bFade = Csng(f8 * f2)
        ElseIf ftmp < 0 Then
            Dim fn,fn2,fn4,fn8 : fn=-ftmp : fn2=fn*fn : fn4=fn2*fn2 : fn8=fn4*fn4
            bFade = Csng(-(fn8 * fn2))
        Else
            bFade = 0
        End If

        If bv > 1 AND bz < 30 or bz > 200 Then
            rolling(b) = True
            PlaySound BallRollStr(b), -1, Csng(bvSq / VolDiv)*.65, bPan, 0, bPitch, 1, 0, bFade
    Else
            If rolling(b) = True Then
                StopSound BallRollStr(b)
                rolling(b) = False
        End If
		If bv > .3 AND bz > 60 And bx < 805 AND by > 610 Then
            rolling(b) = True
            PlaySound MetalRollStr(b), -1, Csng(bvSq / VolDiv)*10, bPan, 0, bPitch, 1, 0, bFade
		End If
		If bz < 55 Then
		Stopsound "fx_metalrolling0"
		Stopsound "fx_metalrolling1"
		Stopsound "fx_metalrolling2"
		Stopsound "fx_metalrolling3"
        End If
End If

    '***Ball Drop Sounds***
    If bvz < -5 and bz > 205 and bx < 362 and by < 554 Then 'height adjust for ball drop sounds
      PlaySound BallDropStr(b), 0, ABS(bvz)/17, bPan, 0, bPitch, 1, 0, bFade
    End If
    If bvz < -5 and bz < 55 and bz > 27 Then 'height adjust for ball drop sounds
      PlaySound BallDropStr(b), 0, ABS(bvz)/17, bPan, 0, bPitch, 1, 0, bFade
    End If
    Next
End Sub
...
Sub OnBallBallCollision(ball1, ball2, velocity)
    ...
    PlaySound "fx_collide", 0, v * v / (VolDiv/VolCol), AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
sub FlipperTimer_Timer()
	...
	Dim aRF2 : aRF2 = RightFlipper2.currentangle
	If aRF2 <> lastRF2Angle Then lastRF2Angle = aRF2 : FlipperRSh1.RotZ = aRF2
End Sub
...
KickerPos = Array(Mk1,Mk2,Mk3,Mk4,Mk5,Mk6,Mk7,Mk8,Mk9,Mk10,Mk11,Mk12,Mk13,Mk14,Mk15,Mk16,Mk17,Mk18,Mk19)
 ...
 Sub HandleMiniPF
 Dim aLF : aLF = LeftFlipper.CurrentAngle
 If aLF <> lastMiniLF Then lastMiniLF = aLF : FlipperL.RotZ = aLF
 Dim aRF : aRF = RightFlipper.CurrentAngle
 If aRF <> lastMiniRF Then lastMiniRF = aRF : FlipperR.RotZ = aRF
 Dim aRF2 : aRF2 = RightFlipper2.CurrentAngle
 If aRF2 <> lastMiniRF2 Then lastMiniRF2 = aRF2 : FlipperR2.RotZ = aRF2
 Dim aG1 : aG1 = Gate1.CurrentAngle
 If aG1 <> lastMiniG1 Then lastMiniG1 = aG1 : PGate1.RotX = aG1
 Dim aG2 : aG2 = Gate2.CurrentAngle
 If aG2 <> lastMiniG2 Then lastMiniG2 = aG2 : PGate2.RotX = aG2
 ...
 kickpadshadow.TransX= 90 * (NewKickerPos-9)/9
