...
End Sub
' Initialize pre-computed constants after table init
Sub InitLISConstants()
    InvTblHeight = 2 / table1.height
    InvTblWidth = 2 / table1.width
End Sub
vpmTimer.AddTimer 100, "InitLISConstants '"
...
Sub PopExit_Hit:Me.DestroyBall:vpmTimer.PulseSwitch 41,100,"AddMystery":End Sub	'41
																				...
																				'43=NOT USED
																				'44=NOT USED
Sub RobotEnter_Hit:Me.DestroyBall:vpmTimer.PulseSwitch 45,100,"AddBRV":Primitive006.z=-40:primitive007.z=-40:Light035.state=2:End Sub	'45=UTrough Robot
...
' Pre-computed inverse multipliers (assigned in Table1_Init)
Dim InvTblHeight, InvTblWidth
...
ReDim BallRollStr(7)
Dim brsI : For brsI = 0 To 7 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next
...
Sub RollingTimer_Timer()
    ...
    For bb = ubBot + 1 to tnob
        rolling(bb) = False
        StopSound BallRollStr(bb)
    Next
   ...
   For bb = 0 to ubBot
        ...
        If bv > 1 AND bz < 30 Then
            rolling(bb) = True
           PlaySound BallRollStr(bb), -1, Csng(bv*bv/2000), AudioPan(ball), 0, bv*20, 1, 0, AudioFade(ball)
        Else
...
sub FlipperTimer_Timer()
    Dim caL : caL = LeftFlipper.currentangle
    Dim caR : caR = RightFlipper.currentangle
	FlipperLSh.RotZ = caL
	FlipperRSh.RotZ = caR
    LFLogo.RotY = caL
    RFlogo.RotY = caR
