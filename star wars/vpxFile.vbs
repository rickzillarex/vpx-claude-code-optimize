...
' Pre-computed trig constants (eliminates Pi() function call + division per dSin/dCos)
Dim CPI : CPI = 4 * Atn(1)
Dim PIover180 : PIover180 = CPI / 180

Function Pi():Pi = CPI:End Function
...
' Pre-computed inverse multipliers for AudioPan/AudioFade (eliminates division per call)
...
' Pre-built string arrays for rolling sounds (eliminates per-frame concatenation)
ReDim BallRollStr(6)
ReDim BallDropStr(6)
Dim brsI : For brsI = 0 To 6 : BallRollStr(brsI) = "fx_ballrolling" & brsI : BallDropStr(brsI) = "fx_ball_drop" & brsI : Next
...
Sub RollingTimer_Timer()
    ...
    Dim bz, bvx, bvy, bvz, bv, bvol, bpitch
    ...
    For b = 0 to ubBot
      ...
      If bv > 1 Then
        ...
        if bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, bvol / 9, AudioPan(BOT(b)), 0, bpitch, 1, 0, AudioFade(BOT(b))
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, bvol / 12, AudioPan(BOT(b)), 0, bpitch + 50000, 1, 0, AudioFade(BOT(b))
        End If
      Else
...
'Flipper Shadows
'***********************
Dim lastLFSAngle : lastLFSAngle = -9999
Dim lastRFSAngle : lastRFSAngle = -9999

' Pre-built BallShadow array at module level (eliminates Array() allocation every frame)
Dim BallShadow
Sub InitBallShadowArray()
...
Sub RealTime_Timer
  ...
  If a <> lastLFSAngle Then lastLFSAngle = a : lfs.RotZ = a
  a = RightFlipper.CurrentAngle
  If a <> lastRFSAngle Then lastRFSAngle = a : rfs.RotZ = a
  BallShadowUpdate
End Sub


Sub BallShadowUpdate()
    If IsEmpty(BallShadow) Then InitBallShadowArray
    Dim BOT, b
    BOT = GetBalls
    Dim ubBot : ubBot = UBound(BOT)
    ...
    For b = 0 to ubBot
        ...
        If bz > 20 Then
            If bz < 200 Then newVis = 1 Else newVis = 0
        Else
            newVis = 0
        End If
