...
Sub LeftFlipper_Timer
    ...
    If Lca >= Lsa - SOSAngle Then LeftFlipper.Strength = FlipperPower * SOSTorque Else LeftFlipper.Strength = FlipperPower
    ...
    If Rca <= Rsa + SOSAngle Then RightFlipper.Strength = FlipperPower * SOSTorque Else RightFlipper.Strength = FlipperPower
...
' Pre-built rolling sound strings (eliminates string concat in hot loop)
ReDim BallRollStr(19)
Dim brsI : For brsI = 0 To 19 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next

' Pre-computed trig constant for SpinKicker (eliminates PI/180 per tick)
Dim PIover180 : PIover180 = Pi / 180

' Flipper shadow last-angle tracking (eliminates redundant COM writes)
...
'****************************************************
Dim sCntrX, sCntrY, Pi, sDegs, sRad, sVel
ReDim SpBall(1)
...
Sub SpinKicker_Timer
    ...
    SpBall(1).z = 25
    sDegs = CubeB.RotZ
    Dim sRad2 : sRad2 = sDegs * PIover180
    Dim sx0 : sx0 = sRad * cos(sRad2) + sCntrX
    Dim sy0 : sy0 = sRad * sin(sRad2) + sCntrY
    SpBall(0).x = sx0
    SpBall(0).y = sy0
    Dim sx1 : sx1 = sCntrX - (sx0 - sCntrX)
    Dim sy1 : sy1 = sCntrY - (sy0 - sCntrY)
    SpBall(1).y = sy1
    SpBall(1).x = sx1
    ...
    sVel = Round(sVel, 3)
    Dim cRot : cRot = CubeB.RotZ
    If cRot <= 0 Then cRot = 360
    If cRot > 360 Then cRot = 1
    cRot = cRot + sVel
    CubeB.RotZ = cRot
    CubePr.RotZ = cRot
    CubePr2.RotZ = cRot
...
Sub OnBallBallCollision(ball1, ball2, velocity)
    ...
    ElseIf ball1.ID = SpBall(1).ID then
        ...
        ElseIf ball2.ID = SpBall(1).ID then
            ...
            If abs(CubeB.RotZ-360) < 180 Then RotAdj = abs(CubeB.RotZ-360) + 180 '*
            Else
                Dim cv : cv = Csng(velocity) : PlaySound "fx_collide", 0, cv * cv / 2000, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
    ...
    sVel = sVel + sin((RotAdj * PIover180) - atn(((SpinBall.Y - BallnPlay.Y) * -1) / (SpinBall.X - BallnPlay.X)) + PiFilln) * velocity / 8
End Sub
