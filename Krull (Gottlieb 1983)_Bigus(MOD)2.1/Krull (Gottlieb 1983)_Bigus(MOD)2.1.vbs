...
'   1. ^10 in AudioFade/AudioPan/Pan eliminated with chain multiply (t2*t2=t4, t4*t4=t8, t8*t2)
'   2. ^2 in Vol/BallVel/OnBallBallCollision replaced with x*x multiply
'   3. Pre-computed table width/height cached at module level
'   4. Pre-built BallRollStr array to avoid string concatenation in rolling loop
'   5. COM property caching in RollingUpdate and BallShadowUpdate
'   6. Early-exit ball loops when UBound(gBOT) = -1
'   7. Shared GetBalls call between BallShadowUpdate and RollingUpdate (called from FlipperTimer)
'=============================================================
...
' Pre-computed table dimensions
Dim TableWidth, TableHeight
TableWidth = Krull.Width
TableHeight = Krull.Height
...
Sub ShooterLaneTop_Hit
	Dim svx: svx = ActiveBall.velx
	Dim svy: svy = ActiveBall.vely
	ShootVel = SQR(svx * svx + svy * svy)
	if ShootVel > 6 then
...
Function BallVel(ball) 'Calculates the ball speed
    ...
    Dim vy: vy = ball.VelY
    Dim vz: vz = ball.VelZ
    BallVel = INT(SQR(vx * vx + vy * vy + vz * vz))
End Function
...
Sub OnBallBallCollision(ball1, ball2, velocity)
    PlaySound("fx_collide"), 0, Csng(velocity * velocity) / 1000, Pan(ball1), 0, Pitch(ball1), 0, 0
End Sub
...
Next
Dim PlasticRollStr(3)
Dim ipr
For ipr = 0 To tnob
    PlasticRollStr(ipr) = "plasticroll" & ipr
Next
InitRolling
...
Sub RollingUpdate()
    ...
    For b = 0 to UBound(BOT)
        ...
        ElseIf BallVel(BOT(b) ) > 1 AND BOT(b).z > 300 Then
			StopSound(BallRollStr(b))
            rolling(b) = True
            PlaySound(PlasticRollStr(b)), -1, Vol(BOT(b)), AudioPan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b))
        Else
