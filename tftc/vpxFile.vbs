...
'**********************************************************************************************************
 ...
 Dim bsTrough, bsBallRelease, bsVuk, bsVukTop, bsPScoop, dtBank, mTombStone
...
Sub Table1_Init
	...
	On Error Resume Next
		...
		.SplashInfoLine = "Tales from the Crypt Data East"
		.HandleMechanics=0
		.HandleKeyboard=0
		.ShowDMDOnly=1
		.ShowFrame=0
     ...
     Set bsVuk = New cvpmBallStack
         ...
         bsVuk.InitKick sw38, 55, 30
		 bsVuk.KickZ=100
         bsVuk.InitExitSnd SoundFX("Popper",DOFContactors), SoundFX("Solenoid",DOFContactors)

     Set bsVukTop = New cvpmBallStack
         bsVukTop.InitSw 0, 52, 0, 0, 0, 0, 0, 0
         bsVukTop.InitKick TopVUK, 165, 33
		 bsVukTop.KickZ=100
         bsVukTop.InitExitSnd SoundFX("Popper",DOFContactors), SoundFX("Solenoid",DOFContactors)

    Set bsPScoop= New cvpmBallStack
        bsPScoop.InitSw 0, 55, 0, 0, 0, 0, 0, 0
        bsPScoop.InitKick sw55, 167, 33
        bsPScoop.KickZ=100
...
Sub Drain_Hit:playsoundAtVol"drain",drain,1:bsTrough.addball 0:Me.DestroyBall:End Sub

 ' Center Vuk
 Sub sw38_Hit:PlaySoundAtVol"popper_ball", ActiveBall, 1:bsVuk.AddBall 0:Me.DestroyBall:End Sub
' Scoop
 Sub sw55_Hit:PlaySoundAtVol"popper_ball", ActiveBall, 1:bspScoop.AddBall 0:Me.DestroyBall:End Sub
'Subway to sw52
 Sub sw53_Hit:vpmTimer.PulseSw 53:Me.DestroyBall:PlaySoundAtVol "popper_ball",sw53,1:bsVukTop.AddBall 0:Controller.switch(52) = True:End Sub
 Sub sw54_Hit:vpmTimer.PulseSw 54:Me.DestroyBall:PlaySoundAtVol "popper_ball",sw54,1:bsVukTop.AddBall 0:Controller.switch(52) = True:End Sub
...
Dim lastTombZ : lastTombZ = -9999
Sub PrimT_Timer
	Dim newZ : newZ = 0
	If rip16.isdropped = false then newZ = 0
	If rip15.isdropped = false then newZ = 11
	If rip14.isdropped = false then newZ = 22
	If rip13.isdropped = false then newZ = 33
	If rip12.isdropped = false then newZ = 44
	If rip11.isdropped = false then newZ = 55
	If rip10.isdropped = false then newZ = 66
	If rip9.isdropped = false then newZ = 77
	If rip8.isdropped = false then newZ = 88
	If rip7.isdropped = false then newZ = 99
	If rip6.isdropped = false then newZ = 110
	If rip5.isdropped = false then newZ = 121
	If rip4.isdropped = false then newZ = 132
	If rip3.isdropped = false then newZ = 144
	If rip2.isdropped = false then newZ = 156
	If rip1.isdropped = false then newZ = 168
    If rip.isdropped = false then newZ = 180
	If newZ <> lastTombZ Then lastTombZ = newZ : tombstone.z = newZ
End Sub
...
LampTimer.Interval = 10 'lamp fading speed (was 5, 100Hz is visually identical)
LampTimer.Enabled = 1
...
Dim InvTWHalf : InvTWHalf = 2 / Table1.Width
Dim InvTHHalf : InvTHHalf = 2 / Table1.Height

' Pre-built rolling sound strings
Dim BallRollStr(8)
Dim brsI : For brsI = 0 To 8 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next

' Pre-built ball drop sound strings
Dim BallDropStr(8)
Dim bdsI : For bdsI = 0 To 8 : BallDropStr(bdsI) = "fx_ball_drop" & bdsI : Next

' Flipper shadow write guards
...
Sub RollingTimer_Timer()
    ...
    For b = 0 to ub
      ...
      If BallVel(BOT(b)) > 1 Then
        ...
        if bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, Vol(BOT(b))/3, AudioPan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b))
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, Vol(BOT(b))/4, AudioPan(BOT(b)), 0, Pitch(BOT(b))+50000, 1, 0, AudioFade(BOT(b))
        End If
      Else
 ...
 ' play ball drop sounds
        ...
        If bvz < -1 and bz < 55 and bz > 27 Then
            PlaySound BallDropStr(b), 0, ABS(bvz)/17, AudioPan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b))
        End If
    Next
End Sub
