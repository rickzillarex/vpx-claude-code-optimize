...
Dim brsInit : For brsInit = 0 To 19 : BallRollStr(brsInit) = "fx_ballrolling" & brsInit : Next
...
Sub RollingUpdate()
    ...
    Dim ball, bvx, bvy, bv, bz, bvz, bpan, bfade

    For b = lob to ubBot
        Set ball = BOT(b)
        bvx = ball.VelX : bvy = ball.VelY : bz = ball.z

		aBallShadow(b).X = ball.X
		aBallShadow(b).Y = ball.Y
        ...
        If bv > 1 Then
            ...
            End If
            rolling(b) = True
            PlaySound BallRollStr(b), -1, ballvol, bpan, 0, ballpitch, 1, 0, bfade
        Else
