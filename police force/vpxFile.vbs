...
' Module-level last-state tracking for DisplayTimer visible guards
Dim lastL23St, lastZ42St, lastZ43St, lastZ44St, lastZ45St, lastZ46St
lastL23St = -1 : lastZ42St = -1 : lastZ43St = -1
lastZ44St = -1 : lastZ45St = -1 : lastZ46St = -1
...
Dim curL23 : curL23 = L23.state
If curL23 <> lastL23St Then Jackpot.visible = curL23 : lastL23St = curL23
Dim curZ42 : curZ42 = Z42.state
If curZ42 <> lastZ42St Then BMan.visible = curZ42 : lastZ42St = curZ42
Dim curZ43 : curZ43 = Z43.state
If curZ43 <> lastZ43St Then GMan.visible = curZ43 : lastZ43St = curZ43
Dim curZ44 : curZ44 = Z44.state
If curZ44 <> lastZ44St Then Oman.visible = curZ44 : lastZ44St = curZ44
Dim curZ45 : curZ45 = Z45.state
If curZ45 <> lastZ45St Then YMan.visible = curZ45 : lastZ45St = curZ45
Dim curZ46 : curZ46 = Z46.state
If curZ46 <> lastZ46St Then Wman.visible = curZ46 : lastZ46St = curZ46
...
' Pre-computed inverse multipliers (initialized in Police_init)
Dim InvPolHeight, InvPolWidth

Sub InitPoliceConstants()
    InvPolHeight = 2 / Police.height
    InvPolWidth = 2 / Police.width
End Sub

Function AudioFade(tableobj) ' Fades between front and back of the table
	Dim tmp
    tmp = tableobj.y * InvPolHeight - 1
    If tmp > 0 Then
...
ReDim BallRollStr(4)
Dim brsI : For brsI = 0 To 4 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next
' Pre-built ball drop strings
ReDim BallDropStr(4)
Dim bdsI : For bdsI = 0 To 4 : BallDropStr(bdsI) = "fx_ball_drop" & bdsI : Next
...
Sub RollingTimer_Timer()
	...
	' play the rolling sound for each ball (inlined BallVel, cached COM)
    Dim ball, bvx, bvy, bvz, bz, bv, bvSq
    For b = 0 to ubBot
      Set ball = BOT(b)
      ...
      If bv > 1 Then
        ...
        bvSq = Csng(bv * bv / 400)
        if bz < 30 Then
          PlaySound BallRollStr(b), -1, bvSq/8, AudioPan(ball), 0, bv*20, 1, 0, AudioFade(ball)
        Else
          PlaySound BallRollStr(b), -1, bvSq/10, AudioPan(ball), 0, bv*20+50000, 1, 0, AudioFade(ball)
        End If
      Else
        ...
        End If
      End If
      ' play ball drop sounds (use cached bvz/bz)
