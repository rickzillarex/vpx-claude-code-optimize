...
Const PropellerMod = 1				'Animate Bi-Plane Propeller when making left ramp (0= no, 1= yes)
...
Sub Table1_KeyDown(ByVal Keycode)
	...
	If keycode = CenterTiltKey Then Nudge 0, 4:PlaySound SoundFX("fx_nudge",0)
...
' OPT: Cache myball properties, use pre-computed BallsizeD6 and InvTWHalf
Sub EnterPoA_timer
	If NOT IsEmpty (myball) Then
		myball.velx = myball.velx + 0.5 * Sgn(PoAPos)
		If myball.VelY<0 Then myball.VelY=1
		Dim mbx : mbx = myball.X
		POABallShadow.X = (mbx - BallsizeD6 + ((mbx - (Table1.Width/2))/7)) + 10
...
' OPT: Replaced TypeName() with VarType + error trapping to avoid COM reflection + string alloc
...
' OPT: Replaced TypeName() with VarType + error trapping
...
Sub UpdateGi(nr,step)
	Dim ii
	Dim stepScale : stepScale = 0.125 * step
	Select Case nr
	Case 0 		'Top Playfield
		...
		End If
		For each ii in GITopSides:ii.IntensityScale = stepScale:Next
		For each ii in GITop:ii.IntensityScale = stepScale:Next
		For each ii in GIBumpers:ii.IntensityScale = stepScale:Next
		For each ii in GIPOA:ii.IntensityScale = stepScale:Next
	Case 1		'Bottom Playfield
		...
		End If
		For each ii in GIBot:ii.IntensityScale = stepScale:Next
		For each ii in GIBotSides:ii.IntensityScale = stepScale:Next
	...
	Case 4		'Return Lane/coin
		...
		End If
		LiteHOF_L.IntensityScale = stepScale:LiteHOF_R.IntensityScale = stepScale
	End Select
End Sub
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	PlaySound("fx_collide"), 0, Csng(velocity * velocity) / 50, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
' OPT: Shared GetBalls — call once, pass to both sound and shadow subs
Sub RealTimeUpdates()
	Dim BOT : BOT = GetBalls
...
Sub RollingSoundUpdateBOT(BOT)
    ...
    For b = 0 to ubBot
        ...
        If bv > 1 Then
			rolling(b) = True
			bz = BOT(b).z
			ballVol = Csng(bv * bv / 400)
			ballPitch = bv * 20
			if bz < 30 Then ' Ball on playfield
						PlaySound BallRollStr(b), -1, ballVol*2, Pan(BOT(b)), 0, ballPitch, 1, 0, AudioFade(BOT(b))
			Else ' Ball on raised ramp
						PlaySound BallRollStr(b), -1, ballVol, Pan(BOT(b)), 0, ballPitch+50000, 1, 0, AudioFade(BOT(b))
				End If
		Else
...
' OPT: Cache flipper angles to reduce COM reads, add delta guards
...
Sub MechsUpdate()
	...
	If lfca <> lastLFAngle Then
		LeftFlipperSh.RotZ = lfca
		lastLFAngle = lfca
	End If
	If rfca <> lastRFAngle Then
		RightFlipperSh.RotZ = rfca
		lastRFAngle = rfca
	End If
End Sub
