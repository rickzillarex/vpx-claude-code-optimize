...
' Optimizations:
'   1. Exponentiation: ^2 replaced with x*x in Vol, BallVel, OnBallBallCollision, Posts_Hit, Rubbers_Hit
'   2. Exponentiation: ^10 replaced with chain multiply in AudioPan, AudioFade
'   3. Pre-computed PIover180 constant for trig conversions in dSin/dCos
'   4. COM property caching in BallShadowUpdate
'   5. Early-exit in RollingSoundUpdate/BallShadowUpdate when no balls
'   6. Pre-built rolling sound strings to avoid per-tick concatenation
'   7. COM property caching in RollingSoundUpdate
...
Dim RollStr
ReDim RollStr(tnob-fakeballs)
Dim rsi_
For rsi_ = 0 To tnob-fakeballs
    RollStr(rsi_) = "fx_ballrolling" & (rsi_+1)
Next
...
Sub RollingSoundUpdate()
    ...
    For b = fakeballs to UBound(BOT)
        ...
        If BallVel(BOT(b)) > 1 AND BOT(b).z < 30 Then
            rolling(idx) = True
			PlaySoundXY RollStr(idx), -1, Vol(BOT(b))/4, 1, BOT(b)
        Else
            If rolling(idx) = True Then
                StopSound(RollStr(idx))
                rolling(idx) = False
            End If
        End If
    Next
