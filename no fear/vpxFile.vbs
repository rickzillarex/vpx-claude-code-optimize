...
'  1. Pre-cached Table1.width/height for Pan/AudioFade
'  2. Eliminated ^10 in Pan/AudioFade with chain multiply
'  3. Eliminated ^2 in Vol, BallVel, OnBallBallCollision
'  4. Inline BallVel/Vol in RollingSoundUpdate loop
'  5. Pre-built rolling sound string array
'  6. Early-exit ball loop when no balls
'  7. BallShadow update uses cached width
' *********************************************************************
...
' Pre-cached table dimensions
Dim NF_TableWidth, NF_TableHeight
NF_TableWidth = Table1.width
NF_TableHeight = Table1.height
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	PlaySound("fx_collide"), 0, Csng(velocity * velocity) / 500, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
Dim NF_RollMax
NF_RollMax = tnob-fakeballs-1
ReDim BallRollStr(NF_RollMax)
Dim iii
For iii = 0 To NF_RollMax
...
Sub RollingSoundUpdate()
    ...
    ' play the rolling sound for each ball (inlined BallVel/Vol)
    For b = fakeballs to UBound(BOT)
        ridx = b-fakeballs
        bvx = BOT(b).VelX : bvy = BOT(b).VelY
        bvel = SQR(bvx*bvx + bvy*bvy)
        If bvel > 1 AND BOT(b).z < 30 AND BOT(b).z > 0 Then
			rolling(ridx) = True
			PlaySound BallRollStr(ridx), -1, Csng((bvx*bvx + bvy*bvy) / 2000)*0.5, Pan(BOT(b)), 0, bvel * 20, 1, 0, AudioFade(BOT(b))
		Else
            If rolling(ridx) = True Then
                StopSound BallRollStr(ridx)
                rolling(ridx) = False
            End If
        End If
...
Sub BallShadowUpdate()
    ...
    For b = fakeballs to UBound(BOT)
		If BOT(b).X < NF_TableWidth/2 Then
			BallShadow(b-fakeballs).X = ((BOT(b).X) - (Ballsize/6) + ((BOT(b).X - (NF_TableWidth/2))/10)) + 10
		Else
			BallShadow(b-fakeballs).X = ((BOT(b).X) + (Ballsize/6) + ((BOT(b).X - (NF_TableWidth/2))/10)) - 10
		End If
