...
'Const BallSize = 50  ' Removed: conflicts with definition in controller.vbs loaded above
'Const BallMass = 1   ' Removed: conflicts with definition in controller.vbs loaded above
...
Function AudioFade(tableobj) ' Fades between front and back of the table
  ...
  ElseIf tmp < 0 Then
    Dim nt,nt2,nt4,nt8 : nt=-tmp : nt2=nt*nt : nt4=nt2*nt2 : nt8=nt4*nt4
...
Function BallRollVol(ball) ' Calculates the Volume of the sound based on the ball speed
   Dim bv : bv = BallVel(ball) : BallRollVol = Csng(bv * bv / (80000 - (79900 * Log(RollVol) / Log(100))))
End Function
...
Sub RollingTimer_Timer()
	...
	Dim ball, bvx, bvy, bv, bvol, bpitch, bpan, bfade, bx, bz, bvz
    For b = 0 to ubBot
		Set ball = BOT(b)
      ...
      If bv > 1 Then
			...
			if MRBallArray(b) = True Then
				StopSound BallRollStr(b)
				PlaySound MetalRollStr(b), -1, bvol * 2 * VolWire, bpan, 0, bpitch, 1, 0, bfade
			Else
				StopSound MetalRollStr(b)
				bz = ball.z
				...
				Else ' Ball on raised ramp
					PlaySound BallRollStr(b), -1, bvol * 2, bpan, 0, bpitch+15000, 1, 0, bfade
				End If
			End If
	   Else
		...
		bvz = ball.VelZ : bz = ball.z
		If bvz < -1 Then
			If bz < 75 Then
				If bz > 27 Then
					PlaySoundAtBOTBallZ BallDropStr(b), ball
				End If
			End If
		End If
		...
		Else
			BallShadow(b).X = (bx + BS_d6 + ((bx - TW_d2)/7)) - 6
		End If
		BallShadow(b).Y = ball.Y + 12
		If Not BallShadow(b).Visible Then BallShadow(b).Visible = True
...
Sub OnBallBallCollision(ball1, ball2, velocity)
  Dim cv : cv = Csng(velocity) : PlaySound("fx_collide"), 0, cv * cv / (VolDiv/VolCol), AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
sub RightFlipper_timer()
	...
	Dim lfEA : lfEA = leftflipper.endangle

	If lfCA = lfEA and LFPress = 1 then
		...
		if GameTime - LFCount < LiveCatch Then
			...
			If lfEA <> LFEndAngle Then leftflipper.endangle = LFEndAngle
		Else
			leftflipper.Elasticity = FElasticity
		end if
	elseif lfCA > lfSA - 0.05  Then
		leftflipper.rampup = SOSRampup
		leftflipper.endangle = LFEndAngle - 3
		leftflipper.Elasticity = FElasticity
		LFCount = 0
	elseif lfCA > lfEA + 0.01 Then
		leftflipper.eostorque = EOST
		leftflipper.eostorqueangle = EOSA
	...
	Dim rfEA : rfEA = rightflipper.endangle

	If rfCA = rfEA and RFPress = 1 then
		...
		if GameTime - RFCount < LiveCatch Then
			...
			If rfEA <> RFEndAngle Then rightflipper.endangle = RFEndAngle
		Else
			rightflipper.Elasticity = FElasticity
		end if
	elseif rfCA < rfSA + 0.05 Then
		rightflipper.rampup = SOSRampup
		rightflipper.endangle = RFEndAngle + 3
		rightflipper.Elasticity = FElasticity
		RFCount = 0
	elseif rfCA < rfEA - 0.01 Then
		rightflipper.eostorque = EOST
		rightflipper.eostorqueangle = EOSA
...
Dim lastLFTAngle : lastLFTAngle = -9999
Dim lastRFTAngle : lastRFTAngle = -9999

Sub UpdateFlippers
	Dim aLF : aLF = LeftFlipper.CurrentAngle
	If aLF <> lastLFAngle Then
		lastLFAngle = aLF
		LeftFlipperShadow.RotY = aLF - 90
	...
	If aRF <> lastRFAngle Then
		lastRFAngle = aRF
		RightFlipperShadow.RotY = aRF - 90
		RightFlipperP.RotY = aRF
	End If

	Dim aLFT : aLFT = LeftFlipperTop.CurrentAngle
	If aLFT <> lastLFTAngle Then
		lastLFTAngle = aLFT
		LeftFlipperTopShadow.RotY = aLFT - 90
		LeftFlipperTopP.RotY = aLFT
	End If

	Dim aRFT : aRFT = RightFlipperTop.CurrentAngle
	If aRFT <> lastRFTAngle Then
		lastRFTAngle = aRFT
		RightFlipperTopShadow.RotY = aRFT - 90
		RightFlipperTopP.RotY = aRFT
	End If
End Sub
...
Class CoRTracker
	...
	Public Sub Update()	'tracks in-ball-velocity
		dim str, b, AllBalls, highestID, ubAll : allBalls = getballs
		ubAll = uBound(allballs)
		if ubAll < 0 then if DebugOn then str = "no balls" : TBPin.text = str : exit Sub else exit sub end if: end if
		for b = 0 to ubAll
			if allballs(b).id >= HighestID then highestID = allballs(b).id
		Next

		if uBound(ballvel) < highestID then redim ballvel(highestID)	'set bounds

		for b = 0 to ubAll
			ballvel(allballs(b).id) = BallSpeed(allballs(b))
			if DebugOn then
				dim s, bs 'debug spacer, ballspeed
				bs = round(BallSpeed(allballs(b)),1)
				if bs < 10 then s = " " else s = "" end if
				str = str & allballs(b).id & ": " & s & bs & vbnewline
			end if
		Next
...
' Pre-computed constants for audio/shadow math
...
ReDim BallRollStr(4)
ReDim MetalRollStr(4)
ReDim BallDropStr(4)
Dim brsI : For brsI = 0 To 4
    BallRollStr(brsI) = "fx_ballrolling" & brsI
    MetalRollStr(brsI) = "fx_metalrolling" & brsI
    BallDropStr(brsI) = "fx_balldrop" & brsI
Next

' Pre-built flasher material strings
ReDim FlasherMatStr(20)
Dim fmsI : For fmsI = 0 To 20 : FlasherMatStr(fmsI) = "Flashermaterial" & fmsI : Next

' Pre-computed RGB white for UpdateMaterial
Dim RGBWhite : RGBWhite = RGB(255,255,255)
...
Sub InitFlasher(nr, col)
	...
	objlight(nr).IntensityScale = 0 : objlit(nr).visible = 0 : objlit(nr).material = FlasherMatStr(nr)
...
Sub FlashFlasher(nr)
	...
	Dim lvl : lvl = FlashLevel(nr)
	Dim lvl2 : lvl2 = lvl * lvl
	Dim lvl3 : lvl3 = lvl2 * lvl
	Dim lvl25 : lvl25 = lvl2 * Sqr(lvl)
	objflasher(nr).opacity = 1000 * lvl25
	objlight(nr).IntensityScale = 0.5 * lvl3
	objbase(nr).BlendDisableLighting = 10 * lvl3
	objlit(nr).BlendDisableLighting = 10 * lvl2
	UpdateMaterial FlasherMatStr(nr),0,0,0,0,0,0,lvl,RGBWhite,0,0,False,True,0,0,0,0
	FlashLevel(nr) = lvl * 0.9 - 0.01
