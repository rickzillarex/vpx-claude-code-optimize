...
Dim BallMod
Dim RubberColor 
Dim DesktopMode: DesktopMode = RaB.ShowDT

EnableBallControl = false 'Change to true to enable manual ball control (or press C in-game) via the arrow keys and B (boost movement) keys

if RaB.showDT = False then ramp001.visible = False:ramp002.visible = False
' ***********************************************************************************************
' OPTIONS
' ***********************************************************************************************

'Const cController 			= 1					' 1=VPinMAME, 2=UVP backglass server, 3=B2S backglass server, 4=B2S backglass server with no VP mech sounds
...
'''''Trough Helpers
	...
	Set gBallShadow(0) = BallShadow1 : Set gBallShadow(1) = BallShadow2
	Set gBallShadow(2) = BallShadow3 : Set gBallShadow(3) = BallShadow4
	Set gBallShadow(4) = BallShadow5 : Set gBallShadow(5) = BallShadow6
	Set gBallShadow(6) = BallShadow7
...
If CustomSideWalls = 0 then
		CustomSideWallsType = Int(Rnd*2)+1
	Else
...
Sub SolAutoPlungerIM(Enabled)
    If Enabled Then
		PlaySoundAt"BallLaunch",Primitive23
        PlungerIM.AutoFire
    End If
End Sub
...
'Playfield Diverter
 ...
 Sub SolDiverter1(enabled)
 	if enabled then
		...
		PlaySoundAt"playfiled_diverter_open",pPFDiverter
 	else
 		Diverter1a.isdropped=False
 		Diverter1b.isdropped=True
		...
		RotatePFDiverter.Enabled = 1
		PlaySoundAt"playfiled_diverter_close",pPFDiverter
 	end if
 end sub
...
'Nell Animation
Dim NellPos, NellDir, NellMoving
NellPos = 75
NellDir = -2
...
Sub NellTimer_Timer()
    StopSound"motor": PlaySoundAt"moter",screw22
	NellPos = NellPos + NellDir
	If NellPos <= 0 Then NellPos = 0:Me.Enabled = 0:RotateSaw.Enabled = 0
	If NellPos >= 150 Then NellPos = 150: Me.Enabled = 0:RotateSaw.Enabled = 0
	pLog.TransX = NellPos
...
Sub MoveNell_timer()
	...
	If NellPos <= 10 Then NellDir = +2:PlaySoundAt"sawmoter",pSawBlade
	If NellPos >= 160 Then NellDir = -2:PlaySoundAt"sawmoter",pSawBlade
...
Sub RotateSaw_timer()
	SawRotY = SawRotY + 5
	If SawRotY >= 360 Then SawRotY = SawRotY - 360
...
Sub SolLFlipper(Enabled)
	If Enabled Then
		PlaySoundAt SoundFX("LeftFlipper_Up",DOFFlippers),LeftFlipper
		LeftFlipper.RotateToEnd
	Else
		PlaySoundAt SoundFX("LeftFlipper_Down",DOFFlippers),LeftFlipper
...
Sub SolRFlipper(Enabled)
	If Enabled Then
		PlaySoundAt SoundFX("RightFlipper_Up",DOFFlippers),RightFlipper
		RightFlipper.RotateToEnd
	Else
		PlaySoundAt SoundFX("RightFlipper_Down",DOFFlippers),RightFlipper
...
Sub sw29_Hit()
	...
	Else
		PlaySoundAtBallVol "SubwayEnterLower",1
	End If
End Sub

Sub sw29kick(Enabled)
	sw29.timerenabled = 1
	PlaySoundAt SoundFX("solenoid",DOFContactors),Primitive19
...
Sub LaserKick_Hit:Me.Kick 0,45:	PlaySoundAt SoundFX("laserkick",DOFContactors),pLaserKick:LKStep = 0:pLaserKick.TransY = 75:Me.TimerEnabled = true:End Sub  '45=strength
...
'Hat Trick up Kick
Sub sw52_Hit():PlaySoundAtBallVol "khattrick_kick_enter",1:End Sub
...
Sub sw52_timer()
	Select Case sw52step
		Case 0:pUpKicker.TransY = 10:PlaySoundAt SoundFX("hattrickkick",DOFContactors),pUpKicker
...
Sub t17_timer()
	Select Case sw17step
		...
		Case 4:pT17.RotX = 3:PlaySoundAt SoundFX("drop_hit",DOFTargets),t17
...
Sub t18_timer()
	Select Case sw18step
		...
		Case 4:pT18.RotX = 3:PlaySoundAt SoundFX("drop_hit",DOFTargets),t18
...
Sub t19_timer()
	Select Case sw19step
		...
		Case 4:pT19.RotX = 3:PlaySoundAt SoundFX("drop_hit",DOFTargets),t19
...
Sub t20_timer()
	Select Case sw20step
		...
		Case 4:pT20.RotX = 3:PlaySoundAt SoundFX("drop_hit",DOFTargets),t20
...
Sub t21_timer()
	Select Case sw21step
		...
		Case 4:pT21.RotX = 3:PlaySoundAt SoundFX("drop_hit",DOFTargets),t21
...
'DT Subs
   Sub DropsUp(Enabled)
		If Enabled Then
			PlaySoundAt SoundFX("DropsUp",DOFTargets),t19
			sw17Dir = 1:sw18Dir = 1:sw19Dir = 1:sw20Dir = 1:sw21Dir = 1
...
Sub CheckDropShadows()
	Dim gi200 : gi200 = (LampState(200) = 1)
	Dim ns

	ns = 0 : If gi200 And target17 = 1 Then ns = 1
	If ns <> lastDropShadowState(0) Then
		lastDropShadowState(0) = ns
		gi12t17.state = ns : gi12ct17.state = ns : gi11t17.state = ns
	End If

	ns = 0 : If gi200 And target18 = 1 Then ns = 1
	If ns <> lastDropShadowState(1) Then
		lastDropShadowState(1) = ns
		gi12t18.state = ns : gi12ct18.state = ns : gi11t18.state = ns
	End If

	ns = 0 : If gi200 And target19 = 1 Then ns = 1
	If ns <> lastDropShadowState(2) Then
		lastDropShadowState(2) = ns
		gi12t19.state = ns : gi12ct19.state = ns : gi11t19.state = ns : gi11ct19.state = ns
	End If

	ns = 0 : If gi200 And target20 = 1 Then ns = 1
	If ns <> lastDropShadowState(3) Then
		lastDropShadowState(3) = ns
		gi12t20.state = ns : gi11t20.state = ns : gi11ct20.state = ns
	End If

	ns = 0 : If gi200 And target21 = 1 Then ns = 1
	If ns <> lastDropShadowState(4) Then
		lastDropShadowState(4) = ns
		gi12t21.state = ns : gi11t21.state = ns : gi11ct21.state = ns
	End If
...
Sub t22_Hit:vpmTimer.PulseSw(22):pt22a.RotY = -5:pt22b.RotY = -5:Target22Step = 0:t22t.Enabled = True:PlaySoundAt SoundFX("standup_hit",DOFTargets),t22:End Sub			
Sub t22t_timer()
...
Sub t23_Hit:vpmTimer.PulseSw(23):pt23a.RotY = -5:pt23b.RotY = -5:Target23Step = 0:t23t.Enabled = True:PlaySoundAt SoundFX("standup_hit",DOFTargets),t23:End Sub			
Sub t23t_timer()
...
Sub t24_Hit:vpmTimer.PulseSw(24):pt24a.RotY = -5:pt24b.RotY = -5:Target24Step = 0:t24t.Enabled = True:PlaySoundAt SoundFX("standup_hit",DOFTargets),t24:End Sub			
Sub t24t_timer()
...
Sub t25_Hit:vpmTimer.PulseSw(25):pt25a.RotZ = 5:pt25b.RotZ = 5:Target25Step = 0:t25t.Enabled = True:PlaySoundAt SoundFX("target",DOFTargets),t24:End Sub			
Sub t25t_timer()
...
Sub t26_Hit:vpmTimer.PulseSw(26):pt26a.RotZ = 5:pt26b.RotZ = 5:Target26Step = 0:t26t.Enabled = True:PlaySoundAt SoundFX("target",DOFTargets),t26:End Sub			
Sub t26t_timer()
...
Sub t27_Hit:vpmTimer.PulseSw(27):pt27a.RotZ = 5:pt27b.RotZ = 5:Target27Step = 0:t27t.Enabled = True:PlaySoundAt SoundFX("target",DOFTargets),t27:End Sub			
Sub t27t_timer()
...
Sub t28_Hit:vpmTimer.PulseSw(28):pt28a.RotZ = 5:pt28b.RotZ = 5:Target28Step = 0:t28t.Enabled = True:PlaySoundAt SoundFX("target",DOFTargets),t28:End Sub			
Sub t28t_timer()
...
Sub Bumper1_hit:vpmTimer.PulseSw(44):PlaySoundAtBumperVol SoundFX("bumper1",DOFContactors),Bumper1,1:End Sub

Sub Bumper2_hit:vpmTimer.PulseSw(45):PlaySoundAtBumperVol SoundFX("bumper2",DOFContactors),Bumper2,1:End Sub
...
Sub sw47_Hit:Controller.Switch(47) = 1:pWayBack_switch.ObjRotX = 5:PlaySoundAt "sensor",sw47:OnWBRamp = 1:End Sub
Sub sw47_Unhit:Controller.Switch(47) = 0:pWayBack_switch.ObjRotX = 0:End Sub

Sub sw48_Hit:Controller.Switch(48) = 1:pRampSwitch1.ObjRotX = -10:PlaySoundAt "sensor",sw48:End Sub
Sub sw48_Unhit:Controller.Switch(48) = 0:pRampSwitch1.ObjRotX = 0:End Sub

Sub sw49_Hit:Controller.Switch(49) = 1:pRockyramp_switch.ObjRotX = -5:PlaySoundAt "sensor",sw49:OnRockyRamp = 1:End Sub
Sub sw49_Unhit:Controller.Switch(49) = 0:pRockyramp_switch.ObjRotX = 0:End Sub

Sub sw50_Hit:Controller.Switch(50) = 1:pRampSwitch.ObjRotX = -10:PlaySoundAt "sensor",sw50:End Sub
...
Sub LeftSlingShot_Slingshot:PlaySoundAt SoundFX("Left_slingshot",DOFContactors),pSlingL:vpmTimer.PulseSw 30:LeftSlingA.Visible = false:LeftSlingB.Visible = True:pSlingL.TransZ = -10:LeftSlingshotStep = 0:Me.TimerEnabled = 1:End Sub
...
Sub RightSlingShot_Slingshot:PlaySoundAt SoundFX("right_slingshot",DOFContactors),pSlingR:vpmTimer.PulseSw 38:RightSlingA.Visible = false:RightSlingB.Visible = True:pSlingR.TransZ = -10:RightSlingshotStep = 0:Me.TimerEnabled = 1:End Sub
...
Dim OnWireRamp, OnGreenRamp, OnBlueRamp

'''''Green ramp
Sub Tgreenramp_start_hit()
OnWireRamp = 1
End Sub

Sub Tgreenramp_end_hit()
OnWireRamp = 0
End Sub
...
Sub Tblueramp_start_hit()
OnWireRamp = 1
End Sub

Sub Tblueramp_end_hit()
OnWireRamp = 0
End Sub
...
Sub KickBallToLane(Enabled)
	If DontKickAnyMoreBalls = 0 then
		PlaySoundAt SoundFX("BallRelease",DOFContactors),screw35
		Kicker1.Kick 70,12
		bstatus = 2
		Kicker1active = 0
		iBall = iBall - 1
...
Sub Drain_hit()
	PlaySoundAt SoundFX("Drain",DOFContactors),pApronB
...
ReDim rolling(tnob)
ReDim RollWoodStr(tnob), RollMetalStr(tnob), RollPlasticStr(tnob)
Dim rsiI : For rsiI = 0 To tnob : RollWoodStr(rsiI) = "fx_Rolling_Wood" & rsiI : RollMetalStr(rsiI) = "fx_Rolling_Metal" & rsiI : RollPlasticStr(rsiI) = "fx_Rolling_Plastic" & rsiI : Next
Dim gBallShadow(6)
Dim lastLFAngle, lastRFAngle
Dim lastLFLogoAngle, lastRFLogoAngle, lastGate3Angle, lastGate4Angle, lastGate5Angle
Dim lastDropShadowState(4)
Dim dssI : For dssI = 0 To 4 : lastDropShadowState(dssI) = -1 : Next
InitRolling
...
Sub CollisionTimer_Timer()
    ...
    For b = 0 to UBound(BOT)
        If BallVel(BOT(b) ) > 1 Then
            ...
            if BOT(b).z < 105 Then 
                        StopSound(RollMetalStr(b)):StopSound(RollPlasticStr(b)):PlaySound(RollWoodStr(b)), -1, Vol(BOT(b) )/10, Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) )

            Else 
				' ***Ball on METAL ramp***
                 If InRect(BOT(b).x, BOT(b).y, 280,620,770,878,794,1490,270,1490) Then
                        StopSound(RollPlasticStr(b)):StopSound(RollWoodStr(b)):PlaySound(RollMetalStr(b)), -1, Vol(BOT(b) ), Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) ) 
                 ElseIf InRect(BOT(b).x, BOT(b).y, 74,360,302,178,324,342,70,570) and BOT(b).z > 150 Then
                       StopSound(RollPlasticStr(b)):StopSound(RollWoodStr(b)):PlaySound(RollMetalStr(b)), -1, Vol(BOT(b) ), Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) ) 
                ElseIf InRect(BOT(b).x, BOT(b).y, 86,983,194,985,184,1567,75,1570) Then
                       StopSound(RollPlasticStr(b)):StopSound(RollWoodStr(b)):PlaySound(RollMetalStr(b)), -1, Vol(BOT(b) ), Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) )
				' ***Ball on PLASTIC ramp***
                Else 
                        StopSound(RollMetalStr(b)):StopSound(RollWoodStr(b)):PlaySound(RollPlasticStr(b)), -1, Vol(BOT(b) ), Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) ) 
                End If
            End If
        ...
        Else
            If rolling(b) = True Then
                StopSound(RollWoodStr(b))
                StopSound(RollPlasticStr(b))
                StopSound(RollMetalStr(b))
                rolling(b) = False
            End If
		...
		If BOT(b).VelZ < 0 and BOT(b).z < 135 and BOT(b).z > 115 Then
    ...
    Next
BallShadowUpdate BOT
Dim curA
curA = LeftFlipper.CurrentAngle
If curA <> lastLFAngle Then lastLFAngle = curA : lfs.RotZ = curA
curA = RightFlipper.CurrentAngle
If curA <> lastRFAngle Then lastRFAngle = curA : rfs.RotZ = curA
End Sub
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	PlaySound("fx_collide"), 0, cv * cv / 200, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub

Sub BallShadowUpdate(BOT)
    Dim b, bz, bvis
    ' hide shadow of deleted balls
    If UBound(BOT)<(tnob-1) Then
        For b = (UBound(BOT) + 1) to (tnob-1)
            gBallShadow(b).visible = 0
        Next
    End If
    ' exit the Sub if no balls on the table
    If UBound(BOT) = -1 Then Exit Sub
    ...
    For b = 0 to UBound(BOT)
        bz = BOT(b).Z
        gBallShadow(b).X = BOT(b).X
        gBallShadow(b).Y = BOT(b).Y + 10
        If bz > 20 And bz < 240 Then bvis = 1 Else bvis = 0
        If gBallShadow(b).visible <> bvis Then gBallShadow(b).visible = bvis
        If bz > 30 Then
            gBallShadow(b).height = bz - 20
            gBallShadow(b).opacity = 110
        Else
            gBallShadow(b).height = bz - 24
            gBallShadow(b).opacity = 90
        End If
    Next
End Sub
