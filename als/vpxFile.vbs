...
' Pre-compute audio inverse multipliers (table COM props available now)
...
Sub GiEffects_timer()
		cachedGI001State = gi001.State  ' cache once per tick for all FadeDisableLighting calls
...
' Cached GI state to avoid repeated COM reads in FadeDisableLighting
Dim cachedGI001State : cachedGI001State = 1

Sub FadeDisableLighting(a, alvlu, alvld, spdu, spdd)

	Select Case cachedGI001State
...
' Pre-computed audio constants (assigned in Table1_Init)
Dim InvTWHalf, InvTHHalf
' These must be assigned inside Table1_Init since table1.width/height are COM properties
' not available at module level. See Table1_Init additions below.
...
' Pre-built rolling sound strings (eliminates per-tick string concat)
...
Sub RollingTimer_Timer()
    ...
    For b = lob to ubBot
        ...
        If bv > 1 Then
            ...
            End If
            rolling(b) = True
            PlaySound BallRollStr(b), -1, ballvol, AudioPan(ball), 0, ballpitch, 1, 0, AudioFade(ball)
        Else
        ...
        ' rothbauerw's Dropping Sounds (uses cached bvz/bz)
        If bvz < -1 and bz < 55 and bz > 27 Then
            PlaySound "fx_balldrop", 0, ABS(bvz)/17, AudioPan(ball), 0, bv * 20, 1, 0, AudioFade(ball)
        End If

        ' jps ball speed control (uses cached bvx/bvy)
...
'Rotate Flipper prims (guarded writes)
	Dim aLF : aLF = LeftFlipper.CurrentAngle
	If aLF <> lastLFAngle Then lastLFAngle = aLF : LeftFlipperPrim.RotZ = aLF
	Dim aRF : aRF = RightFlipper.CurrentAngle
	If aRF <> lastRFAngle Then lastRFAngle = aRF : RightFlipperPrim.RotZ = aRF
End Sub
...
RightFlipper.timerinterval=10  ' was 1 — 100Hz is visually identical to 1000Hz
...
Sub CheckLiveCatch(ball, Flipper, FCount, parm) 'Experimental new live catch
	...
	Dim fx : fx = Flipper.x  ' cache COM read

	if CatchTime <= LiveCatch and parm > 6 and ABS(fx - ball.x) > LiveDistanceMin and ABS(fx - ball.x) < LiveDistanceMax Then
