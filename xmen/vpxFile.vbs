...
' Pre-computed constants (assigned after table objects available)
Dim InvTWHalf, InvTHHalf

' Pre-built rolling sound strings
ReDim BallRollStr(6)
ReDim BallDropStr(6)
Dim brsI : For brsI = 0 To 6
...
Const EnableFlipperShadows = False
Const EnableIceManShadow = False
...
'********************
  ...
  Sub Table_Init
    ...
    ' Initialize pre-computed constants (table dimensions now available)
...
LeftNightCrawler.blenddisablelighting = 1 
Else 
LeftNightCrawler.blenddisablelighting = 0.3
    End If
End Sub
...
RightNightCrawler.blenddisablelighting = 1
Else
RightNightCrawler.blenddisablelighting = 0.3
    End If
End Sub
...
Sub solDiscMotor(Enabled)
    if Enabled Then    
        ttSpinner.MotorOn = True : Playsound SoundFX("disc_noise",DOFGear),-1,0.04,0,0,-60000,0,0,AudioFade(f120b1):f120b1.state = 1
    Else   
        ttSpinner.MotorOn = False : Stopsound "disc_noise":f120b1.state = 0
    end If
end sub
...
Sub sw52_Hit:Controller.Switch(52) = 1:PlaySoundAt "Gate", ActiveBall:IceManHold = IceManHold + 1:sw52.TimerInterval=200:vpmtimer.addtimer 300, "sw52.TimerEnabled=1'":End Sub
Sub sw52_Timer
...
Sub SolRFlipper(Enabled)
     If Enabled Then
         ...
         RightFlipper.RotateToEnd:PlaySound SoundFX("FlipperUpUR",DOFFlippers),0,2,AudioPan(RightFlipper1),0,6000,0,0,AudioFade(RightFlipper1):RightFlipper1.RotateToEnd
     Else
         PlaySoundAt SoundFX("FlipperDown",DOFFlippers), f131b
        RightFlipper.RotateToStart:PlaySound SoundFX("FlipperDownUR",DOFFlippers),0,1,AudioPan(RightFlipper1),0,6000,0,0,AudioFade(RightFlipper1):RightFlipper1.RotateToStart
     End If
 End Sub  
...
Sub LampMod(nr, object)
    Dim lmFade : lmFade = FadingLevel(nr)/128
    Dim lmState : lmState = LampState(nr)
    Dim tn : tn = TypeName(object)
    If tn = "Light" Then
        Object.IntensityScale = lmFade
        Object.State = lmState
    ElseIf tn = "Flasher" Then
        Object.IntensityScale = lmFade
        Object.visible = lmState
    ElseIf tn = "Primitive" Then
        Object.DisableLighting = lmState
    End If
End Sub
...
Sub UpdateFlipperLogo
    ...
    If aLF <> lastLFAngle Then
        lastLFAngle = aLF
        LFLogo.RotY = aLF
        if EnableFlipperShadows then FlipperLSh.RotZ = aLF
    End If
    ...
    If aRF <> lastRFAngle Then
        lastRFAngle = aRF
        RFlogo.RotY = aRF
        if EnableFlipperShadows then FlipperRSh.RotZ = aRF
    End If
    ...
    If aRF1 <> lastRF1Angle Then
        lastRF1Angle = aRF1
        RFlogo1.RotY = aRF1
        if EnableFlipperShadows then FlipperRSh1.RotZ = aRF1
    End If
End Sub
...
backpanel.opacity = 300
            GI_State = 1       
        Else   
            Table.ColorGradeImage= "ColorGrade_1"
backpanel.opacity = 100
            GI_State = 0
        End If 
    End Select
    UpdateGI
...
Sub PlaySoundAtVol(sound, tableobj, Vol)
    ...
    Else
        PlaySound sound, 1, Vol, AudioPan(tableobj)
    End If
End Sub
...
Function Vol(ball) ' Calculates the Volume of the sound based on the ball speed
    Dim bv : bv = BallVel(ball) : Vol = Csng(bv * bv / 4000)
End Function
...
Sub RollingTimer_Timer()
    ...
    Dim ball, bvx, bvy, bv, bvol, bpitch, bpan, bfade, bz, bvz
    For b = 0 to ubBot
      Set ball = BOT(b)
      bvx = ball.VelX : bvy = ball.VelY
      bv = Int(Sqr(bvx * bvx + bvy * bvy))
      If bv > 1 Then
        bvol = Csng(bv * bv / 4000)
        bpitch = bv * 20
        bpan = AudioPan(ball)
        ...
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, bvol*.5, bpan, 0, bpitch+50000, 1, 0, bfade
        End If
      Else
...
Sub OnBallBallCollision(ball1, ball2, velocity)
  ...
  next
  If Table.VersionMinor > 3 OR Table.VersionMajor > 10 Then
    Dim cv : cv = Csng(velocity) : PlaySound("fx_collide"), 0, cv * cv / 80, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
  else
    Dim cv2 : cv2 = Csng(velocity) : PlaySound("fx_collide"), 0, cv2 * cv2 / 80, AudioPan(ball1), 0, Pitch(ball1), 0, 0
  end if
End Sub
...
Sub BallShadow_Timer()
    ...
    If ubBot<(tnob-1) Then
        For b = (ubBot + 1) to (tnob-1)
            If BallShadow(b).visible Then BallShadow(b).visible = 0
        Next
    End If
    ' exit the Sub if no balls on the table
    If ubBot = -1 Then Exit Sub
    ...
    For b = 0 to ubBot
        ...
        If bz > 20 Then
            If bz < 140 Then newVis = 1 Else newVis = 0
        Else
            newVis = 0
        End If
...
Sub PrimT_Timer
    Dim s28 : s28 = l28a.State
    Dim v28 : If s28 = 1 Then v28 = 1 Else v28 = 0
    f28a.visible = v28 : f28b.visible = v28 : f28c.visible = v28
    Dim s29 : s29 = l29a.State
    Dim v29 : If s29 = 1 Then v29 = 1 Else v29 = 0
    f29a.visible = v29 : f29b.visible = v29 : f29c.visible = v29
End Sub
