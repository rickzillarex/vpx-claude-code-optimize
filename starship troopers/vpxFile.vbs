...
Sub InitLamps()
    ...
    For x = 0 to 200
        ...
        FlashSpeedUp(x) = 1.28   ' faster speed when turning on (scaled for 16ms interval, was 0.4 at 5ms)
        FlashSpeedDown(x) = 0.64 ' slower speed when turning off (scaled for 16ms interval, was 0.2 at 5ms)
...
Sub FlashFlasher(nr)
	...
	Dim lvl : lvl = FlashLevelb(nr)
	Dim lvl2 : lvl2 = lvl * lvl
	Dim lvl3 : lvl3 = lvl2 * lvl
	objflasher(nr).opacity = 1000 * lvl2 * Sqr(lvl)
	objlight(nr).IntensityScale = 0.03 * lvl3
	objbase(nr).BlendDisableLighting = 0.4 + 10 * lvl3
	...
	If ObjActive(nr) Then
		FlashLevelb(nr) = lvl * 0.7 - 0.01
	Else
		FlashLevelb(nr) = lvl * 0.9 - 0.01
	End If
	If not ObjActive(nr) Then
		If FlashLevelb(nr) < 0 Then objflasher(nr).TimerEnabled = False : objflasher(nr).visible = 0 : objlit(nr).visible = 0
	End If
	If ObjActive(nr) Then
		If FlashLevelb(nr) < 0.5 Then FlashLevelb(nr) = 1
	End If
End Sub
...
Dim lastMFAngle : lastMFAngle = -9999
...
sub FlipperTimer_Timer()
	...
	Dim aMF : aMF = RightFlipper1.currentangle
	If aMF <> lastMFAngle Then
		lastMFAngle = aMF
		FlipperRSh1.RotZ = aMF
		MFlogo.RotZ = aMF + 120
	End If
End Sub
