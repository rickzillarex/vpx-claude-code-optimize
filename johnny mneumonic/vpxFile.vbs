...
if JM.ShowDT = false then SideRailL.visible = 0:SideRailR.visible = 0
...
Sub UpdateGi(nr,step)
	Dim ii
If Step>=7 Then JM.ColorGradeImage = "ColorGradeLUT256x16_extraConSat":Else JM.ColorGradeImage = "ColorGrade_" & (step+1):End If
	Select Case nr
...
Sub AddLamp(nr, object)
    ...
    Select Case LampState(nr)
        ...
        Case 0 'off
			...
			If vt = 9 Then ' vbObject — check once, use cached type string
				Dim tn0 : tn0 = TypeName(object)
				If tn0 = "Light" Then
					Object.State = 0
				ElseIf tn0 = "Flasher" Then
					Object.IntensityScale = FlashLevel(nr)
				ElseIf tn0 = "Primitive" Then
        ...
        Case 1 ' on
			...
			If vt = 9 Then
				Dim tn1 : tn1 = TypeName(object)
				If tn1 = "Light" Then
					Object.State = 1
				ElseIf tn1 = "Flasher" Then
					Object.IntensityScale = FlashLevel(nr)
				ElseIf tn1 = "Primitive" Then
...
Sub RealTimeUpdate
	...
	If aLF <> lastLFAngle Then
	    lastLFAngle = aLF
	    FlipperL.objrotz = aLF + 1
	    lfs.RotZ = aLF
	End If
	If aRF <> lastRFAngle Then
	    lastRFAngle = aRF
	    FlipperR.objrotz = aRF + 1
	    rfs.RotZ = aRF
	End If
...
' Pre-computed table dimension inverses (set at module level from JM object)
Dim JMWidth, JMHeight, InvJMWHalf, InvJMHHalf
JMWidth = JM.width : JMHeight = JM.height
InvJMWHalf = 2 / JMWidth : InvJMHHalf = 2 / JMHeight
...
ReDim BallDropStr(5)
Dim bdsI : For bdsI = 0 To 5 : BallDropStr(bdsI) = "fx_ball_drop" & bdsI : Next

' Flipper bat angle tracking for guarded writes
Dim lastLFAngle : lastLFAngle = -9999
Dim lastRFAngle : lastRFAngle = -9999

Function AudioFade(tableobj)
	Dim tmp
    tmp = tableobj.y * InvJMHHalf - 1
    If tmp > 0 Then
    ...
    For b = 0 to ubBot
      ...
      If bv > 1 Then
        ...
        if bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, Csng(bv * bv / 6000), AudioPan(BOT(b)), 0, bv * 20, 1, 0, AudioFade(BOT(b))
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, Csng(bv * bv / 6000) * 0.5, AudioPan(BOT(b)), 0, bv * 20 + 50000, 1, 0, AudioFade(BOT(b))
        End If
      Else
