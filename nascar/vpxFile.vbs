...
'  1. FlipperTricks timer 1ms -> 10ms
'  2. Cache flipper COM properties in FlipperTricks/FlipperNudge
'  3. Inline FlipperTrigger - cache Flipper.x/.y/.currentangle/.Length
'  4. Distance ^2 -> x*x (eliminate Sqr + ^2 pattern)
'  5. Eliminated ^10 in AudioFade/AudioPan with chain multiply
'  6. Eliminated ^2 in Vol, BallVel, VolPlayfieldRoll
'  7. Eliminated ^3 in VolPlayfieldRoll with chain multiply
'  8. Eliminated ^2 in PitchPlayfieldRoll with multiply
'  9. Inline BallVel in RollingUpdate loop
' 10. Pre-built rolling sound string array
' 11. Cache chgLamp locals in LampTimer_Timer
' 12. GI visibility delta-write guards in LampTimer
' *********************************************************************
...
Dim prevGi10State : prevGi10State = -1
Dim prevL40State : prevL40State = -1
Dim prevL32State : prevL32State = -1
...
Sub FlipperNudge(Flipper1, Endangle1, EOSNudge1, Flipper2, EndAngle2)
	...
	Dim gBOT
	Dim f1Angle, f2Angle
	f1Angle = Flipper1.currentangle
	f2Angle = Flipper2.currentangle
	gBOT = GetBalls

	If f1Angle = Endangle1 And EOSNudge1 <> 1 Then
		EOSNudge1 = 1
		If f2Angle = EndAngle2 Then
...
Function FlipperTrigger(ballx, bally, Flipper)
	Dim DiffAngle, fx, fy, fAngle, fLen
	fx = Flipper.x : fy = Flipper.y : fAngle = Flipper.currentangle : fLen = Flipper.Length
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	PlaySound (snd), 0, CSng(velocity * velocity / 200) * BallWithBallCollisionSoundFactor * VolumeDial, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
...
Sub RollingUpdate()
	...
	' play the rolling sound for each ball (inlined BallVel/VolPlayfieldRoll/PitchPlayfieldRoll)
	For b = 0 To UBound(gBOT)
		bvx = gBOT(b).VelX : bvy = gBOT(b).VelY
		bvel = Int(Sqr(bvx*bvx + bvy*bvy))
		If bvel > 1 And gBOT(b).z < 30 Then
			rolling(b) = True
			bv = bvel
			PlaySound BallRollStr(b), -1, RollingSoundFactor * 0.0005 * CSng(bv*bv*bv) * BallRollVolume * VolumeDial, AudioPan(gBOT(b)), 0, bv*bv*15, 1, 0, AudioFade(gBOT(b))
		Else
