...
'   2. Flipper COM property caching in LeftFlipper_Timer
'   3. Exponentiation: ^2 replaced with x*x in Vol, BallVel, OnBallBallCollision
'   4. Exponentiation: ^10 replaced with chain multiply in Pan, AudioFade
'   5. Pre-built rolling sound strings to avoid per-tick concatenation
'   6. COM property caching in RollingUpdate (ball X/Y/Z/VelX/VelY)
'   7. UBound caching in RollingUpdate
...
Sub LeftFlipper_Timer 'flipper's tricks timer
    Dim LFCurAngle, LFStartAng, LFEndAng
    Dim RFCurAngle, RFStartAng, RFEndAng
    LFCurAngle = LeftFlipper.CurrentAngle
    LFStartAng = LeftFlipper.StartAngle
    LFEndAng = LeftFlipper.EndAngle
    RFCurAngle = RightFlipper.CurrentAngle
    RFStartAng = RightFlipper.StartAngle
    RFEndAng = RightFlipper.EndAngle

'Start Of Stroke Flipper Stroke Routine : Start of Stroke for Tap pass and Tap shoot
    If LFCurAngle >= LFStartAng - SOSAngle Then LeftFlipper.Strength = FlipperPower * SOSTorque else LeftFlipper.Strength = FlipperPower : End If

'End Of Stroke Routine : Livecatch and Emply/Full-Charged EOS
	If LeftFlipperOn = 1 Then
		If LFCurAngle = LFEndAng then
...
'Start Of Stroke Flipper Stroke Routine : Start of Stroke for Tap pass and Tap shoot
    If RFCurAngle <= RFStartAng + SOSAngle Then RightFlipper.Strength = FlipperPower * SOSTorque else RightFlipper.Strength = FlipperPower : End If

'End Of Stroke Routine : Livecatch and Emply/Full-Charged EOS
 	If RightFlipperOn = 1 Then
		If RFCurAngle = RFEndAng Then
...
Dim brs_
For brs_ = 0 To tnob
    BallRollStr(brs_) = "fx_ballrolling" & brs_
Next

Sub RollingUpdate()
    Dim BOT, b, ballpitch, ballvol, speedfactorx, speedfactory, ubBot
    BOT = GetBalls
    ubBot = UBound(BOT)
    ...
    For b = lob to ubBot
        ...
        aBallShadow(b).X = bx
        aBallShadow(b).Y = by
        aBallShadow(b).Height = bz - Ballsize / 2 + 1

        If BallVel(BOT(b)) > 1 Then
            If bz < 30 Then
                ballpitch = Pitch(BOT(b))
                ballvol = Vol(BOT(b))
            ElseIf bz > 50 AND bz < 190 Then
...
Sub OnBallBallCollision(ball1, ball2, velocity)
    ...
    PlaySound("fx_collide"), 0, cv * cv / 2000, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
