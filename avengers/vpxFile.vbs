...
' Optimizations:
'   1. FlipperTricks timer interval: 1ms changed to 10ms
'   2. Flipper COM property caching in FlipperTricks and FlipperNudge
'   3. Exponentiation: ^2 replaced with x*x in Distance, Vol, Volz, BallVel, PitchPlayfieldRoll, TargetBouncer, OnBallBallCollision
'   4. Exponentiation: ^10 replaced with chain multiply in AudioFade, AudioPan
'   5. Exponentiation: ^3 replaced with x*x*x in VolPlayfieldRoll
'   6. Pre-computed PIover180 constant for trig conversions in dSin/dCos/Radians
'   7. Pre-built rolling sound strings to avoid per-tick concatenation
'   8. COM property caching in RollingUpdate (ball X/Y/Z)
'   9. UBound caching in RollingUpdate
'  10. Early-exit in RollingUpdate when no balls on table
...
' 							Table Options
'************************************************************************
SideBladeMod = 1					' 0 = Black Side Blades , 1 = Custom Blades, 2 = Alt Custom Blades
InstMod = 0							' 0 = Stern Intruction Cards, 1 = French Intruction Cards, 2 = German Intruction Cards
...
Dim SideBladeMod, GlassMod,DMDDecalmod,InstMod, LogoMod, PosterMod, AltBackglass, TopperMod, x
...
Sub Table1_Init
    ...
    Set bsHole = New cvpmSaucer
        bsHole.InitKicker HulkEject, 62, 25, 15, 2
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	ElseIf ball2.ID = SpBall(1).ID then
		...
		If abs(CubeB.RotZ-360) < 180 Then RotAdj = abs(CubeB.RotZ-360) + 180		'*
	Else
		Dim cv : cv = Csng(velocity) : PlaySound("fx_collide"), 0, cv * cv / 2000, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
...
Sub Table1_KeyDown(ByVal keycode)
	...
	If Keycode = StartGameKey Then Controller.Switch(16) = 1:SoundStartButton
...
Function BallSpeed(ball) 'Calculates the ball speed
	Dim bsx : bsx = ball.VelX : Dim bsy : bsy = ball.VelY : Dim bsz : bsz = ball.VelZ
	BallSpeed = SQR(bsx * bsx + bsy * bsy + bsz * bsz)
End Function
...
Function Distance(ax,ay,bx,by)
	Dim dx_ : dx_ = ax - bx : Dim dy_ : dy_ = ay - by
	Distance = Sqr(dx_ * dx_ + dy_ * dy_)
End Function
...
Sub FlipperTricks (Flipper, FlipperPress, FCount, FEndAngle, FState)
	...
	If absFca > Abs(fsa) - 0.05 Then
		If FState <> 1 Then
			...
			FState = 1
		End If
	ElseIf absFca <= Abs(fea) And FlipperPress = 1 Then
		...
		If FState <> 2 Then
			...
			FState = 2
		End If
	ElseIf absFca > Abs(fea) + 0.01 And FlipperPress = 1 Then
		If FState <> 3 Then
			Flipper.eostorque = EOST
			Flipper.eostorqueangle = EOSA
...
Sub TargetBouncer(aBall,defvalue)
	...
	If TargetBouncerEnabled = 1 And aball.z < 30 Then
		...
		aBall.velx = Sgn(aBall.velx) * Sqr(Abs((vel * vel - aBall.velz * aBall.velz) / (1 + vratio * vratio)))
...
' Pre-built rolling sound strings (avoids per-tick concatenation)
Dim BallRollStr
ReDim BallRollStr(tnob)
Dim brs_
For brs_ = 0 To tnob
    BallRollStr(brs_) = "BallRoll_" & brs_
Next
...
Sub RollingUpdate()
	...
	For b = 0 To ubBot
		bz = BOT(b).z
		If BallVel(BOT(b)) > 1 And bz < 30 Then
			rolling(b) = True
			PlaySound (BallRollStr(b)), - 1, VolPlayfieldRoll(BOT(b)) * BallRollVolume * VolumeDial, AudioPan(BOT(b)), 0, PitchPlayfieldRoll(BOT(b)), 1, 0, AudioFade(BOT(b))
		Else
		...
		If AmbientBallShadowOn = 0 Then
			If bz > 30 Then
				BallShadowA(b).height = bz - BallSize / 4		'This is technically 1/4 of the ball "above" the ramp, but it keeps it from clipping the ramp
			Else
