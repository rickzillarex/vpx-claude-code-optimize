' VPX Performance Optimization Applied
' Optimizations:
'   1. Exponentiation: ^2 replaced with x*x in Vol, BallVel, OnBallBallCollision
'   2. Exponentiation: ^10 replaced with chain multiply in Pan
'   3. Pre-computed PIover180 constant for trig conversions
'   4. Pre-built rolling sound strings at module level
'   5. COM property caching in RollingUpdate (ball.X, ball.Y, ball.Z)
'   6. COM property caching in BallShadowUpdate
'   7. Early-exit in RollingUpdate/BallShadowUpdate when no balls
...
if DesktopMode = False then rightrail.visible=0:leftrail.visible=0
'**********************************************************************************************************
'Solenoid Call backs
'**********************************************************************************************************
...
SolModCallBack(6)   = "SolFlasher6" 								'Right Green Dome
SolModCallBack(12)  = "SolFlasher12" 								'Left Blue Dome
SolModCallBack(13)  = "SolFlasher13" 								'Right Red Dome
SolCallBack(25)  = "SetLamp 125," 								'PF light insert
SolCallBack(26)  = "SetLamp 126," 								'PF light insert
SolModCallBack(27)  = "SolFlasher27" 								'Spot Light
...
SolModCallBack(32)  = "SolFlasher32" 								'Left yellow Dome
...
Sub Table1_Init
	...
	On Error Resume Next
		...
		.SplashInfoLine = "Austin Powers - Stern 2001"
		.HandleMechanics=0
		.HandleKeyboard=0
		.ShowDMDOnly=1
		.ShowFrame=0
	...
	With bsScoop
	    .InitKicker Sw46b, 46, 180.5, 100, 30
		.InitSounds "popper_ball", SoundFX(SSolenoidOn,DOFContactors), SoundFX("popper",DOFContactors)
...
Sub SolTimeMachine(Enabled)
   ...
   TimeMachineP.blenddisablelighting = 0.8
      Else
       TimeMachineTimer.Enabled=0
   TimeMachineP.disablelighting=0
  End If
End Sub
...
Sub SolFlasher6(m):m = m/128:FlasherL6.state = m:Primitive68.blenddisablelighting = m+0.1:End Sub
Sub SolFlasher12(m):m = m/128:FlasherL12.state = m:Primitive10.blenddisablelighting = m+0.1:End Sub
Sub SolFlasher13(m):m = m/128:FlasherL13.state = m:Primitive9.blenddisablelighting = m+0.1:End Sub
Sub SolFlasher32(m):m = m/128:FlasherL32.state = m:Primitive11.blenddisablelighting = m+0.1:End Sub
Sub SolFlasher27(m):m = m/32:FlasherLight27.state = m:FlasherLight27a.state = m:F127a.blenddisablelighting = m:End Sub
...
Sub Trig1_Hit:Primitive1.blenddisablelighting = 0.8:Primitive4.blenddisablelighting = 0.4:End Sub
Sub Trig1_unHit:Primitive1.disablelighting = 0 :Primitive4.disablelighting = 0:End Sub

'Bumpers
Sub Bumper1_Hit:vpmTimer.PulseSw 49 : playsound SoundFX("LeftBumper_Hit",DOFContactors) : End Sub
Sub Bumper2_Hit:vpmTimer.PulseSw 50 : playsound SoundFX("RightBumper_Hit",DOFContactors) : End Sub
Sub Bumper3_Hit:vpmTimer.PulseSw 51 : playsound SoundFX("TopBumper_Hit",DOFContactors) : End Sub

'Stand Up Targets
' Left Banks Targets
Sub Sw25_Hit:vpmTimer.PulseSw 25 :MoveTarget25 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
Sub Sw26Hit:vpmTimer.PulseSw 26 :MoveTarget26 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
Sub Sw27_Hit:vpmTimer.PulseSw 27 :MoveTarget27 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
Sub Sw28_Hit:vpmTimer.PulseSw 28 :MoveTarget27 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
Sub Sw29_Hit:vpmTimer.PulseSw 29 :MoveTarget29 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
Sub Sw30_Hit:vpmTimer.PulseSw 30 :MoveTarget30 :PlaySound SoundFX("fx_bumper",DOFContactors), 0, 1, 0.15, 0.15:End Sub
...
Sub sw35_hit:vpmTimer.pulseSw 35 : playsound"fx_chapa" : End Sub

Sub sw37_Hit:Controller.Switch(37)=1 : playsound"fx_gate4" : End Sub
Sub sw37_unHit:Controller.Switch(37)=0:End Sub


Sub sw38_hit:vpmTimer.pulseSw 38 : playsound"fx_gate4" : End Sub

Sub sw41_Hit:Controller.Switch(41)=1 End Sub
Sub sw41_unHit:Controller.Switch(41)=0:End Sub

Sub sw42_hit:vpmTimer.pulseSw 42 :playsound "fx_gate4": End Sub
...
BallInGunRadius = SQR((PCannon.X - Sw45.X)^2 + (PCannon.Y - Sw45.Y)^2)
 ...
 Sub SolLaserBeam(Enabled)
	If Enabled Then
		PlaySound SoundFX("CannoShot", DOFContactors)
...
End Sub
 ...
 Sub UpdateLserGun(acurrpos, aspeed, alastpos)
	...
	PCannon.RotY = GPos
	If Not IsEmpty(BallInGun) Then
		Dim gAngle : gAngle = (GPos+30)*PIover180
		BallInGun.X = PCannon.X - BallInGunRadius * Cos(gAngle)
		BallInGun.Y = PCannon.Y - BallInGunRadius * Sin(gAngle)
	End If
 End Sub

Sub Sw45_hit(): StopSound "fx_ramp_enter3": PlaySound "popper_ball": controller.switch(45) = 1:Set BallInGun = ActiveBall:End Sub
...
Sub Sw24_Hit():Sw24pos=0:vpmTimer.PulseSw 24:Me.TimerInterval=5:Me.TimerEnabled=1: End Sub
 ...
 Sub Sw24_Timer()	
 	Select Case Sw24pos
        Case 0: Sw24P.y=657:PlaySound"fx_chapa"         
        Case 1: Sw24P.y=647 
        Case 2: Sw24P.y=637 
        Case 3: Sw24P.y=627 
        Case 4: Sw24P.y=637 
...
Sub UpdateEvil(aNewPos,aSpeed,aLastPos)
    ...
    If aNewPos > 3 then
        sw24.IsDropped = 0
Sw24P.blenddisablelighting = 0.8
        'DR_Evil.IsDropped = 0
    Else
        sw24.Isdropped = 1
Sw24P.disablelighting = 0
        'DR_Evil.IsDropped = 1
    End If
End Sub
...
Sub UpdateLamps
	...
	NFadeL 51, Light51
	NFadeL 52, Light52
FadeDisableLighting 53, l53, 10
    NFadeLm 53, F53 'Fire Button
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "table1" is the name of the table
    ...
    If tmp > 0 Then
        Dim t2 : t2 = tmp * tmp : Dim t4 : t4 = t2 * t2 : Dim t5 : t5 = t4 * tmp
        Pan = Csng(t5 * t5)
    Else
        Dim nt : nt = -tmp : Dim n2 : n2 = nt * nt : Dim n4 : n4 = n2 * n2 : Dim n5 : n5 = n4 * nt
        Pan = Csng(-(n5 * n5))
    End If
End Function
...
Function BallVel(ball) 'Calculates the ball speed
    Dim bvx : bvx = ball.VelX : Dim bvy : bvy = ball.VelY
    BallVel = INT(SQR(bvx * bvx + bvy * bvy))
End Function
...
Dim BallRollStr(5)
Dim iStr
For iStr = 0 To 5 : BallRollStr(iStr) = "fx_ballrolling" & iStr : Next
InitRolling

Sub InitRolling:Dim i:For i=0 to (tnob-1):rolling(i) = False:Next:End Sub

Sub RollingUpdate()
    Dim BOT, b, ballpitch, ubBot
    BOT = GetBalls
    ubBot = UBound(BOT)
    ...
    For b = fakeballs to ubBot
        If BallVel(BOT(b) ) > 1 Then
           ...
           If bz < 30 Then
            ...
            End If
            rolling(b) = True
            PlaySound BallRollStr(b), -1, Vol(BOT(b))/2, Pan(BOT(b)), 0, ballpitch, 1, 0
        Else
...
BallShadow = Array (BallShadow1,BallShadow2,BallShadow3,BallShadow4,BallShadow5,BallShadow6)
    ...
    For b = 0 to ubBot
        Dim bx : bx = BOT(b).X : Dim by : by = BOT(b).Y : Dim bzs : bzs = BOT(b).Z
		BallShadow(b).X = bx
		ballShadow(b).Y = by + 10
        If bzs > 20 and bzs < 140 Then
...
Sub Rubbers_Hit(idx):PlaySound "fx_rubber_hit_2", 0, Vol(ActiveBall)*4, pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0:End Sub
...
Sub Trigger2_Hit:PlaySound "fx_chapa" End Sub
...
Sub LREnter_Hit():If ActiveBall.VelY < 0 Then PlaySound "fx_ramp_enter1":End If:End Sub			'ball is going up
Sub LREnter_UnHit():If ActiveBall.VelY > 0 Then StopSound "fx_ramp_enter1":End If:End Sub		'ball is going down
Sub LREnter1_Hit():PlaySound "fx_ramp_enter2":End Sub
Sub LREnter2_Hit():PlaySound "fx_ramp_enter3":End Sub
...
Sub RREnter2_Hit():PlaySound "fx_ramp_enter3":End Sub
