...
'  1. Pre-cached table width for Pan() (eliminates per-call COM read)
'  2. Eliminated ^10 in Pan() with chain multiply
'  3. Eliminated ^2 in Vol(), BallVel(), OnBallBallCollision()
'  4. Inline BallVel in RollingSound loop (reduce function call overhead)
'  5. Early-exit ball loop when no balls in RollingSound
'  6. Pre-built rolling sound string array
'  7. Cache chgLamp locals in LampTimer_Timer
' *********************************************************************
...
Dim BallShadow
BallShadow = Array(BallShadow001, BallShadow002, BallShadow003, BallShadow004, BallShadow005, BallShadow006, BallShadow007, BallShadow008)
Const shift = 27
...
Sub sw25_Hit:vpmTimer.PulseSw 25:PlaySound SoundFX("fx_target",DOFContactors):sw25p.transY = -10:Me.TimerEnabled = 1:End Sub
Sub sw25_Timer:sw25p.transY = 0:Me.TimerEnabled = 0:End Sub 
Sub sw26_Hit:vpmTimer.PulseSw 26:PlaySound SoundFX("fx_target",DOFContactors):sw26p.transY = -10:Me.TimerEnabled = 1:End Sub
Sub sw26_Timer:sw26p.transY = 0:Me.TimerEnabled = 0:End Sub 
Sub sw27_Hit:vpmTimer.PulseSw 27:PlaySound SoundFX("fx_target",DOFContactors):sw27p.transY = -10:Me.TimerEnabled = 1:End Sub
...
Sub Sw40_Hit:vpmTimer.PulseSw 40:PlaySound SoundFX("fx_target",DOFContactors):End sub
...
Sub Sw56_Hit:vpmTimer.PulseSw 56:PlaySound SoundFX("fx_target",DOFContactors):End Sub
...
Sub LampTimer_Timer()
    ...
    If Not IsEmpty(chgLamp) Then
        For ii = 0 To UBound(chgLamp)
            ...
            FadingLevel(idx) = val + 4      'actual fading step
        Next
    End If
    UpdateLamps
End Sub
...
' Pre-cached table width
Dim LAH_TableWidth
LAH_TableWidth = LastActionHero.width
...
Function Pan(ball) 									' Calculates the pan for a ball based on the X position on the table.
    Dim tmp, tmp2											' "table1" is the name of the table
    tmp = ball.x * 2 / LAH_TableWidth-1
    If tmp> 0 Then
...
Sub RollingSound()
    ...
    Dim bvx, bvy, bvel, bvol
    For b = 0 to UBound(BOT)
        bvx = BOT(b).VelX : bvy = BOT(b).VelY
        bvel = SQR(bvx*bvx + bvy*bvy)
        If bvel > 1 AND BOT(b).z < 30 Then
            rolling(b) = True
            bvol = Csng((bvx*bvx + bvy*bvy) / 2000)
            PlaySound BallRollStr(b), -1, bvol, Pan(BOT(b)), 0, bvel * 20, 1, 0
        Else
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	PlaySound("fx_collide"), 0, Csng(velocity * velocity / 500), Pan(ball1), 0, Pitch(ball1), 0, 0
End Sub
'******************************************************
'* SOUND EFFECTS **************************************
'******************************************************
