...
Dim InvTblHeight, InvTblWidth, InvFlashScale, HalfTblWidth, BallSizeDiv6
ReDim BallRollStr(5)

Sub InitPrecomputedConstants()
    InvTblHeight = 2 / table1.height
    InvTblWidth = 2 / table1.width
    HalfTblWidth = Table1.Width / 2
    BallSizeDiv6 = Ballsize / 6
    InvFlashScale = 1 / (2.55 * 100)
    Dim brsI
    For brsI = 0 To 5
...
Sub Flash20(level)
	If Level > 0 Then
		F20.IntensityScale = level * InvFlashScale
		F20.State = 1
	Else
		F20.State = 0
	End If
...
Sub Flash22(level)
	If Level > 0 Then
	Dim sc22 : sc22 = level * InvFlashScale
	F22.IntensityScale = sc22
	F22b.IntensityScale = sc22
	F22c.IntensityScale = sc22
	F22.State = 1
	F22b.State = 1
	F22c.State = 1
	Else
...
Sub Flash30(level)
	If Level > 0 Then
	Dim sc30 : sc30 = level * InvFlashScale
	F30.IntensityScale = sc30
	F30b.IntensityScale = sc30
	F30c.IntensityScale = sc30
	F30d.IntensityScale = sc30
	F30e.IntensityScale = sc30
	F30.State = 1
	F30b.State = 1
	F30c.State = 1
	F30d.State = 1
...
Sub Flash31(level)
	If Level > 0 Then
	F31.IntensityScale = level * InvFlashScale
	F31.State = 1
	Else
	F31.State = 0
	End If
...
Sub Flash32(level)
	If Level > 0 Then
	F32.IntensityScale = level * InvFlashScale
	F32.State = 1
	Else
	F32.State = 0
	End If
...
Dim lastGI1St, lastGI2St, lastGI3St, lastL24St, lastL32St, lastL40St, lastL48St, lastL56St
lastGI1St = -1 : lastGI2St = -1 : lastGI3St = -1 : lastL24St = -1
lastL32St = -1 : lastL40St = -1 : lastL48St = -1 : lastL56St = -1
...
Sub MiscTimer_Timer
	...
	' Guard visible writes - only write when state changes
	Dim gs1 : gs1 = GISpot1.State
	If gs1 <> lastGI1St Then GISpot1b.visible = gs1 : lastGI1St = gs1
	Dim gs2 : gs2 = GISpot2.State
	If gs2 <> lastGI2St Then GISpot2b.visible = gs2 : lastGI2St = gs2
	Dim gs3 : gs3 = GISpot3.State
	If gs3 <> lastGI3St Then GISpot3b.visible = gs3 : lastGI3St = gs3
	Dim ls24 : ls24 = L24.state
	If ls24 <> lastL24St Then l24B.visible = ls24 : lastL24St = ls24
	Dim ls32 : ls32 = L32.state
	If ls32 <> lastL32St Then L32b.visible = ls32 : lastL32St = ls32
	Dim ls40 : ls40 = L40.state
	If ls40 <> lastL40St Then L40b.visible = ls40 : lastL40St = ls40
	Dim ls48 : ls48 = L48.state
	If ls48 <> lastL48St Then L48b.visible = ls48 : lastL48St = ls48
	Dim ls56 : ls56 = L56.state
	If ls56 <> lastL56St Then L56b.visible = ls56 : lastL56St = ls56
...
Sub RollingTimer_Timer()
	...
	' play the rolling sound for each ball (inlined BallVel/Vol/Pitch)
    Dim ball, bvx, bvy, bv, bz
    For b = 0 to ubBot
        Set ball = BOT(b)
        ...
        If bv > 1 AND bz < 30 Then
            rolling(b) = True
            PlaySound BallRollStr(b), -1, CSng(Round(bv/50,2)), AudioPan(ball), 0, bv*20, 1, 0, AudioFade(ball)
        Else
...
Sub BallShadowUpdate_timer()
    ...
    ' exit the Sub if no balls on the table
    If ubBot = -1 Then Exit Sub
    ' render the shadow for each ball (cached COM reads)
    Dim bx, by, bz, newVis
    For b = 0 to ubBot
        bx = BOT(b).X : by = BOT(b).Y : bz = BOT(b).Z
        If bx < HalfTblWidth Then
            BallShadow(b).X = (bx - BallSizeDiv6 + ((bx - HalfTblWidth)/7)) + 13
        Else
            BallShadow(b).X = (bx + BallSizeDiv6 + ((bx - HalfTblWidth)/7)) - 13
        End If
