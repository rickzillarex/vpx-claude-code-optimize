...
'							TABLE OPTIONS
'************************************************************************
Const VaultEdition=0				'Show new lower pf plate, LED flashers and apron stickers, these were added in the Vault Edition models produced in 2017
...
'Pre-computed constants (require table object to be ready)
	InvTWHalf = 2 / ACDC.Width
	InvTHHalf = 2 / ACDC.Height
	HalfTableW = ACDC.Width / 2
	BallsizeD6 = Ballsize / 6
...
' 						CANNON ENTER
'************************************************************************
Dim PIover180 : PIover180 = 3.1415926535 / 180

' Pre-computed constants for audio functions
Dim InvTWHalf
Dim InvTHHalf
Dim HalfTableW
Dim BallsizeD6

' Pre-built rolling sound strings (eliminates string concat per frame)
ReDim BallRollStr(6)
Dim brsI : For brsI = 0 To 6 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next

' Shared GetBalls result for RealTimeUpdates (avoids 2 GetBalls per frame)
Dim gBOT

' GI last-value tracking (eliminates redundant computation when GI stable)
Dim lastGIRed : lastGIRed = -1
Dim lastGIBlue : lastGIBlue = -1
Dim lastGILowPF : lastGILowPF = -1
Dim lastGIWhite : lastGIWhite = -1

' FIRE button last-state tracking
Dim lastFIRER : lastFIRER = -1
Dim lastFIREG : lastFIREG = -1
Dim lastFIREB : lastFIREB = -1
...
End Sub
 ...
 Sub UpdateCannon_Timer()
	...
	Dim gLaser : gLaser = -(GPos-90)
	LaserR.objRotZ = gLaser
	LaserR1.objRotZ = gLaser
	If Not IsEmpty(BallInGun) Then
		Dim gRad : gRad = GPos * PIover180
		BallInGun.X = Cannon_assyM.X - BallInGunRadius * Cos(gRad)
		BallInGun.Y = Cannon_assyM.Y - BallInGunRadius * Sin(gRad)
...
Sub Flipper1_timer
	...
	Dim f2ca : f2ca = Flipper2.currentangle
	SolPlung.TransX = f2ca
	Supp_Angus.RotY = f1ca
	Supp_Malcolm.RotY = f1ca
	Supp_Brian.RotY = f1ca
	Supp_Cliff.RotY = f1ca
	Figure_Angus.RotY = f1ca
	Dim legRad : legRad = f1ca * PIover180
	Figure_AngusLeg.X = Supp_Angus.X + LegRotRadius * Sin(legRad)
	Figure_AngusLeg.Z = Supp_Angus.Z + LegRotRadius * Cos(legRad)
	Figure_Malcolm.RotY = f1ca
	Figure_Brian.RotY = f1ca
	Figure_Cliff.RotY = f1ca
	Figure_DrumL.RotY = f2ca
	Figure_DrumR.RotY = -f2ca
	If f1ca > 10 Then LegCase = 1: Flipper2.TimerInterval= 10: Flipper2.TimerEnabled= 1
	If f1ca = 35 Then Flipper1.RotateToStart: Flipper2.RotateToStart
	If f1ca = 0 Then Me.TimerEnabled = 0
End Sub
...
Dim sin5deg : sin5deg = sin(5 * PIover180)
Dim cos5deg : cos5deg = cos(5 * PIover180)

Sub BellMove_Timer
	Dim bRot : bRot = Spinner5.currentangle
	Bell.RotX = bRot : Bell_Support.RotX = bRot : Bell_Log.RotX = bRot
	Dim bRad : bRad = bRot * PIover180
	Dim bArm : bArm = (Bell.Z-50) * sin(bRad)
	NewtonBall.X = Bell.X + bArm * sin5deg
	NewtonBall.Y = Bell.Y + bArm * cos5deg
	NewtonBall.Z = Bell.Z - (Bell.Z-50) * cos(bRad)
End Sub
...
Sub BellK_timer
	...
	Dim bw3 : bw3 = brakew * brakew * brakew
	Dim bRot : bRot = -(cos(rotat+90)*100)/bw3
	Bell.rotX = bRot : Bell_Support.RotX = bRot : Bell_Log.RotX = bRot
	Dim bRad : bRad = bRot * PIover180
	Dim bArm : bArm = (Bell.Z-50) * sin(bRad)
	NewtonBall.X = Bell.X + bArm * sin5deg
	NewtonBall.Y = Bell.Y + bArm * cos5deg
	NewtonBall.Z = Bell.Z - (Bell.Z-50) * cos(bRad)
	If (bRot < 16 AND bRot > 8) OR (bRot < -8 AND bRot > -16) Then Controller.Switch(47) = 1 : Else Controller.Switch(47) = 0 : End If
	If (bRot < 1 AND bRot > -1 AND ccc>500) OR (BellHit AND NOT BellExit) Then Me.TimerEnabled=0:ccc=0:BellMove.Enabled=1
End Sub
...
'LED Flames Tunnel
    LampModFlasher 151, l151
    LampModFlasher 152, l152
    LampModFlasher 153, l153
    LampModFlasher 154, l154
    LampModFlasher 155, l155
    LampModFlasher 156, l156
    LampModFlasher 157, l157
    LampModFlasher 158, l158

'FIRE button
Dim fR : fR = LampState(59) : Dim fG : fG = LampState(60) : Dim fB : fB = LampState(61)
If fR <> lastFIRER Or fG <> lastFIREG Or fB <> lastFIREB Then
    lastFIRER = fR : lastFIREG = fG : lastFIREB = fB
    Dim fCol : fCol = RGB(255*fR, 255*fG, 255*fB)
    l63.colorfull = fCol : l63a.colorfull = fCol : l63b.colorfull = fCol : l63c.colorfull = fCol
    Dim fSt : If fR Or fG Or fB Then fSt = 1 Else fSt = 0
    l63.state = fSt : l63a.state = fSt : l63b.state = fSt : l63c.state = fSt
End If
...
'VPM returns an 0-100 range value
	...
	RGBLEDFlasher l40a,LampState(92), LampState(91), LampState(90)		'tunes n stuff (metal reflection)
	...
	RGBLEDFlasher l39a,LampState(128), LampState(127), LampState(126)	'right top lane (metal reflection)

'Flashers
'VPM returns an 0-255 range value
	LampModFlasher 177, f17		'Train
	LampModFlasher 177, f17a	'Train
	LampModFlasher 177, f17b	'Train
	LampModFlasher 177, f17c	'Train
	LampMod 177, f17r		'Train (light)
	LampModFlasher 179, f19		'Bottom Arch
	LampModFlasher 179, f19a	'Bottom Arch
	LampModFlasher 180, f20		'Left Ramp
	LampModFlasher 181, f21		'Yellow Dome
	LampModFlasher 181, f21a	'Yellow Dome
	LampModFlasher 181, f21c	'Yellow Dome
	LampModFlasher 181, f21b	'Yellow Dome
	LampModFlasher 181, f21bDT	'Yellow Dome
	LampModLight 181, f21r		'Yellow Dome (light)
	LampModFlasher 182, f22 	'Backbox
	LampModFlasher 182, f22b 	'Backbox
	LampModFlasher 182, f22c 	'Backbox
	LampModLight 182, f22r 		'Backbox (light)
	LampModFlasher 183, f23 	'Top Eject
	LampModFlasher 183, f23a 	'Top Eject
	LampModFlasher 185, f25a 	'Bumper Flash
	LampModFlasher 185, f25 	'Bumper Flash
	LampModFlasher 186, f26 	'Bell Arrow
	LampModFlasher 187, f27 	'Thunder Left
	LampModFlasher 187, f27a	'Thunder Left
	LampModFlasher 187, f27b	'Thunder Left
	LampModFlasher 187, f27c	'Thunder Left
	LampModFlasher 188, f28 	'Thunder Center
	LampModFlasher 188, f28a	'Thunder Center
	LampModFlasher 188, f28b	'Thunder Center
	LampModFlasher 188, f28c	'Thunder Center
	LampModFlasher 189, f29 	'Thunder Right
	LampModFlasher 189, f29a	'Thunder Right
	LampModFlasher 189, f29b	'Thunder Right
	LampModFlasher 189, f29c	'Thunder Right
	LampModFlasher 190, f30		'Right Ramp
	LampModFlasher 191, f31 	'Red Dome
	LampModFlasher 191, f31a	'Red Dome
	LampModFlasher 191, f31b	'Red Dome
	LampModFlasher 191, f31c	'Red Dome
	LampModFlasher 191, f31bDT	'Red Dome
	LampModFlasher 191, f31bDT1	'Red Dome
	LampModFlasher 191, f31d	'Red Dome
	LampModLight 191, f31r		'Red Dome (light)

End Sub

Sub UpdateGIs
	Dim gR : gR = LampState(130)
	Dim gB : gB = LampState(132)
	Dim gL : gL = LampState(134)
	Dim gW : gW = LampState(136)

	If gR <> lastGIRed Then
		lastGIRed = gR
		Dim rScale : rScale = gR / 50
		Dim rInt : rInt = gR * 1.5
		Dim rBDL : rBDL = 8 * rScale
		Dim rMat : rMat = RGB(255*rScale, 255*rScale, 255*rScale)
		For each bulb in GI_Red: bulb.Intensity = rInt: Next
		For Each bulb in BLRLights: bulb.IntensityScale = rScale: Next
		bulb2.BlendDisableLighting = rBDL
		bulb4.BlendDisableLighting = rBDL
		bulb6.BlendDisableLighting = rBDL
		MaterialColor "TrainRed", rMat
	End If

	If gB <> lastGIBlue Then
		lastGIBlue = gB
		Dim bScale : bScale = gB / 50
		Dim bInt : bInt = gB * 1.5
		Dim bBDL : bBDL = 8 * bScale
		Dim bMat : bMat = RGB(255*bScale, 255*bScale, 255*bScale)
		For each bulb in GI_Blue: bulb.Intensity = bInt: Next
		For Each bulb in BLBLights: bulb.IntensityScale = bScale: Next
		bulb1.BlendDisableLighting = bBDL
		bulb3.BlendDisableLighting = bBDL
		bulb5.BlendDisableLighting = bBDL
		bulb7.BlendDisableLighting = bBDL
		MaterialColor "TrainBlue", bMat
	End If

	If gL <> lastGILowPF Then
		lastGILowPF = gL
		Dim lScale : lScale = gL / 50
		light16.intensityScale = lScale
		light19.intensityScale = lScale
		light44.intensityScale = lScale
		light45.intensityScale = lScale
		For each bulb in GI_LowPF: bulb.Intensity = gL * 3: Next
	End If

	If gW <> lastGIWhite Then
		lastGIWhite = gW
		Dim wScale : wScale = gW / 50
		For each bulb in GI_White: bulb.IntensityScale = wScale: Next
	End If

	If gR = 0 AND gB = 0 AND gW = 0 Then
		ACDC.ColorGradeImage="ColorGrade_off":For Each bulb in BandMembersPoster: bulb.IntensityScale=0:Next
	ElseIf gR > 0 AND gB = 0 AND gL > 0 AND gW = 0 Then ACDC.ColorGradeImage="ColorGrade_red"
	ElseIf gR = 0 AND gB > 0 AND gL = 0 AND gW = 0 Then ACDC.ColorGradeImage="ColorGrade_blue"
	Else
...
Sub RGBLED (object,red,green,blue)
	Dim c : c = RGB(2.5*red, 2.5*green, 2.5*blue)
	object.colorfull = c
	object.state = 1
End Sub

Sub RGBLEDFlasher (object,red,green,blue)
...
Sub LampMod(nr, object)
	...
	Dim vt : vt = VarType(object)
	If vt = 9 Then
		On Error Resume Next
		Object.State = LampState(nr)
		If Err Then Err.Clear : Object.visible = LampState(nr)
		On Error Goto 0
	End If
End Sub

Sub LampModLight(nr, object)
	Object.IntensityScale = ModulationLevel(nr)/128
	On Error Resume Next
	Object.State = LampState(nr)
	If Err Then Err.Clear : Object.visible = LampState(nr)
...
Sub RollingSoundUpdate()
	...
	If ubBot < (tnob - 1) Then
		Dim bs
		For bs = (ubBot + 1) to (tnob-1)
			rolling(bs) = False
			StopSound BallRollStr(bs)
		Next
	End If
    If ubBot = fakeballs-1 Then Exit Sub

    Dim b, bvx, bvy, bz, bv, ballvol, ballpitch
    For b = 0 to ubBot
        ...
        If bv > 1 Then
			...
			If bz < 30 Then
				PlaySound BallRollStr(b), -1, ballvol*.8, Pan(BOT(b)), 0, bv*20, 1, 0, AudioFade(BOT(b))
			Else
				PlaySound BallRollStr(b), -1, ballvol*.4, Pan(BOT(b)), 0, bv*20+50000, 1, 0, AudioFade(BOT(b))
			End If
		Else
...
Sub BallShadowUpdate()
	...
	If ubBot < (tnob-1) Then
		Dim bs
		For bs = (ubBot + 1) to (tnob-1)
			BallShadow(bs).visible = 0
		Next
	End If
    If ubBot = -1 Then Exit Sub
    Dim b, bx, by, bz, newVis
    For b = 0 to ubBot
		bx = BOT(b).X : by = BOT(b).Y : bz = BOT(b).Z
		If bx < HalfTableW Then
			BallShadow(b).X = (bx - BallsizeD6 + ((bx - HalfTableW)/7)) + 10
		Else
			BallShadow(b).X = (bx + BallsizeD6 + ((bx - HalfTableW)/7)) - 10
		End If
	    BallShadow(b).Y = by + 20
...
Sub InitOptions
	...
	Select Case InstrCardType
		...
		Case 2
			Cards.image= "ACDC-Helen-Card"
	End Select
	If HelenMod=0 Then
		Lower_PF.image = "low-pf"
		SpinnerT1.image= "spinner"
	Else
		Lower_PF.image = "low-pf_Helen"
