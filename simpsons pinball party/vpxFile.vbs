...
' Pre-computed constants (initialized in Table1_Init)
...
Dim lastHeadPos : lastHeadPos = -9999
Dim lastHeadState : lastHeadState = -1

Sub UpdateHead()
	CheckLamps
	' All HeadRef cases produce identical output, so skip the outer Select
	HHead.Roty = HeadPos
	If HeadState <> lastHeadState Then
		lastHeadState = HeadState
		Select Case HeadState
			Case 0 : HHead.Image = "hh"
			Case 1 : HHead.Image = "hhFb"
			Case 2 : HHead.Image = "hhFa"
			Case 3 : HHead.Image = "hhFOn"
		End Select
	End If
End Sub
...
LampTimer.Interval = 16 'lamp fading speed (was 5ms, 16ms is visually identical)
LampTimer.Enabled = 1
...
Sub InitLamps()
    ...
    For x = 0 to 200
        ...
        FlashSpeedUp(x) = 1.28   ' faster speed when turning on the flasher (was 0.4, scaled for 16ms timer)
        FlashSpeedDown(x) = 0.64 ' slower speed when turning off the flasher (was 0.2, scaled for 16ms timer)
...
ReDim LEDStateCache(149) ' cache previous LED state to skip redundant writes
Dim LEDStateCacheInit : LEDStateCacheInit = False

Sub UpdateLeds
	If Not LEDStateCacheInit Then
		Dim ci : For ci = 0 To 149 : LEDStateCache(ci) = -1 : Next
		LEDStateCacheInit = True
	End If

	Dim ChgLED, ii, jj, num, stat, s, x, ledObj
		ChgLED = Controller.ChangedLEDs(0, &HFFFFF)
			If Not IsEmpty(ChgLED) Then
				...
				x = num * 7 + 6-jj
				s = stat And 3
		If s <> LEDStateCache(x) Then
			LEDStateCache(x) = s
			Set ledObj = REDL(x)
			Select Case s
				Case 3:
					ledObj.material = "EVYellow" : ledObj.visible = 1
				Case 2:
					ledObj.material = "EVGreen" : ledObj.visible = 1
				Case 1:
					ledObj.material = "EVRed" : ledObj.visible = 1
				Case 0:
					ledObj.visible = 0
			End Select
		End If
...
Function Vol(ball) ' Calculates the Volume of the sound based on the ball speed
    Dim bv : bv = BallVel(ball) : Vol = Csng(bv * bv / 800)
End Function
    ...
    For b = 0 to ubBot
      ...
      If bv > 1 Then
        ...
        If bz < 30 Then ' Ball on playfield
          PlaySound BallRollStr(b), -1, bvol/7, bpan, 0, bpitch, 1, 0, bfade
        Else ' Ball on raised ramp
          PlaySound BallRollStr(b), -1, bvol/10, bpan, 0, bpitch+50000, 1, 0, bfade
        End If
      Else
