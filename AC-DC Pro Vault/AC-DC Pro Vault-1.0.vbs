...
' ==========================================
Dim PIover180 : PIover180 = 3.14159265358979 / 180
Dim InvTWHalf, InvTHHalf
Dim BallsizeD6 : BallsizeD6 = Ballsize / 6
...
ReDim BallRollStr(4)
Dim brsI : For brsI = 0 To tnob - 1 : BallRollStr(brsI) = "fx_ballrolling" & brsI : Next
...
' OPT: Pre-compute table dimension inverse multipliers
...
BallInGunRadius = SQR((Cannon_assyM.X - Sw45.X)*(Cannon_assyM.X - Sw45.X) + (Cannon_assyM.Y - Sw45.Y)*(Cannon_assyM.Y - Sw45.Y))
GPos = 110: GDir = -1
...
' OPT: Use pre-computed PIover180 instead of Pi/180
 Sub UpdateCannon_Timer()
	...
	Dim gPosNeg : gPosNeg = -(GPos-90)
	LaserR.objRotZ = gPosNeg
	LaserR1.objRotZ = gPosNeg
...
Sub sw36_timer
	Dim bellAngle : bellAngle = -SpinnerBell.CurrentAngle
	Bell.RotX = bellAngle
	Bell_Support.RotX = bellAngle: Bell_Support1.RotX= bellAngle
End Sub
...
' OPT: Cache chglamp(ii,0) and chglamp(ii,1) into locals
Sub LampTimer_Timer()
...
' RGB Leds
' OPT: Replaced TypeName() with VarType() check — avoids COM reflection + string alloc
Sub RGBLED (object,red,green,blue)
	If VarType(object) = 9 Then
		' Could be Light or Flasher — check by attempting state property
		On Error Resume Next
		Dim testState : testState = object.state
...
' OPT: Removed TypeName() calls — replaced with VarType check
Sub LampMod(nr, object)
	Dim modScale : modScale = ModulationLevel(nr)/128
	Dim ls : ls = LampState(nr)
	Object.IntensityScale = modScale
	' Use error trapping to detect type instead of TypeName (avoids COM reflection)
	On Error Resume Next
	Dim testState : testState = object.state
	If Err.Number = 0 Then
		' It's a Light
...
' OPT: ^2 replaced with bv*bv
Function Vol(ball) ' Calculates the Volume of the sound based on the ball speed
    Dim bv : bv = BallVel(ball)
    Vol = Csng(bv * bv / 2000)
End Function

' OPT: ^10 replaced with chained multiply, division replaced with pre-computed inverse
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table
    ...
    ElseIf tmp < 0 Then
        Dim nt : nt = -tmp
        Dim n2,n4,n8 : n2=nt*nt : n4=n2*n2 : n8=n4*n4
        Pan = Csng(-(n8 * n2))
    Else
        Pan = 0
    End If
...
' OPT: ^2 replaced with vx*vx + vy*vy
...
' OPT: Shared GetBalls — call once, pass to both RollingSoundUpdate and BallShadowUpdate
Sub RealTimeUpdates
	Dim BOT : BOT = GetBalls
	RollingSoundUpdateBOT BOT
	BallShadowUpdateBOT BOT
...
' OPT: Accepts pre-fetched BOT array, inlined BallVel/Vol/Pitch,
'       pre-built string arrays, COM property caching
Sub RollingSoundUpdateBOT(BOT)
    Dim b
    ...
    Dim bvx, bvy, bv, bz, ballVol, ballPitch
    ...
    For b = 0 to ubBot
        ...
        If bv > 1 Then
			...
			if bz < 30 Then ' Ball on playfield
						PlaySound BallRollStr(b), -1, ballVol*.8, Pan(BOT(b)), 0, ballPitch, 1, 0, AudioFade(BOT(b))
			Else ' Ball on raised ramp
						PlaySound BallRollStr(b), -1, ballVol*.4, Pan(BOT(b)), 0, ballPitch+50000, 1, 0, AudioFade(BOT(b))
				End If
		Else
...
' Keep original signature for backward compatibility if called elsewhere
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	PlaySound("fx_collide"), 0, Csng(velocity * velocity) / 200, Pan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
' OPT: Accepts pre-fetched BOT array, COM property caching, guard .visible writes,
'       pre-computed BallsizeD6 and table width half
Sub BallShadowUpdateBOT(BOT)
    Dim b
    Dim ubBot : ubBot = UBound(BOT)
    ...
    For b = 0 to ubBot
		...
		bz = BOT(b).Z
		If bx < halfW Then
			BallShadow(b).X = (bx - BallsizeD6 + ((bx - halfW)/7)) + 10
		Else
			BallShadow(b).X = (bx + BallsizeD6 + ((bx - halfW)/7)) - 10
		End If
